...
int
SOAP_FMAC2
soap_ssl_client_context_cert(struct soap *soap, unsigned short flags, const char *certfile, const char *keyfile, const char *password, const char *cafile, const char *capath, const char *randfile)
{ soap->certfile = certfile;
  soap->keyfile = keyfile;
  soap->password = password;
  soap->cafile = cafile;
  soap->capath = capath;
...
{ long flags;
  ...
  if (soap->certfile) {
  // this has been added to be able to supply separate certificate and key files
  // which is needed for GRIDSAM support
    if (!SSL_CTX_use_certificate_chain_file(soap->ctx, soap->certfile))
    ...
    if (!SSL_CTX_check_private_key(soap->ctx))
      return soap_set_receiver_error(soap, "SSL error", "Can't read key file", SOAP_SSL_ERROR);
