...
namespace irods_file_adaptor {
	...
	int icommands::imeta(int argc, char **argv) {
...
//		}
		...
		*/
		...
		while (keepGoing) {
		  int status;
		  status=doCommand(Conn, myEnv, cmdToken);
		 ...
		 firstTime=0;
		  ...
		  if (keepGoing) {
		 getInput(Conn, cmdToken, maxCmdTokens);
		  }
		}
	 ...
	 */
	void icommands::printGenQueryResults(rcComm_t *Conn, int status, genQueryOut_t *genQueryOut,
				 char *descriptions[])
	{
	   int i, j;
	...
	int icommands::showDataObj(rcComm_t *Conn, char *name, char *attrName, int wild)
	{
...
//	   printf("AVUs defined for dataObj %s:\n",name);
	...
	*/
	int icommands::showColl(rcComm_t *Conn, char *name, char *attrName, int wild)
	{
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
...
//	   printf("AVUs defined for collection %s:\n",name);
	...
	*/
	int icommands::showResc(rcComm_t *Conn, char *name, char *attrName, int wild)
	{
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
...
//	   printf("AVUs defined for resource %s:\n",name);
	...
	*/
	int icommands::showRescGroup(rcComm_t *Conn, char *name, char *attrName, int wild)
	{
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
...
//	   printf("AVUs defined for resource group %s:\n",name);
	...
	*/
	int icommands::showUser(rcComm_t *Conn, rodsEnv myEnv, char *name, char *attrName, int wild)
	{
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
...
//	   printf("AVUs defined for user %s#%s:\n",userName, userZone);
	 ...
	 */
	int icommands::queryDataObj(rcComm_t *Conn, char *cmdToken[]) {
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[20];
	   int i1b[20];
	 ...
	 */
	int icommands::queryCollection(rcComm_t *Conn, char *attribute, char *op, char *value) {
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
	   int i1b[10];
	 ...
	 */
	int icommands::queryResc(rcComm_t *Conn, char *attribute, char *op, char *value) {
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
	   int i1b[10];
	 ...
	 */
	int icommands::queryRescGroup(rcComm_t *Conn, char *attribute, char *op, char *value) {
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
	   int i1b[10];
	 ...
	 */
	int icommands::queryUser(rcComm_t *Conn, rodsEnv myEnv, char *attribute, char *op, char *value) {
	   genQueryInp_t genQueryInp;
	   genQueryOut_t *genQueryOut;
	   int i1a[10];
	   int i1b[10];
	...
	/*
	 Modify (copy) AVUs
	 */
	int icommands::modCopyAVUMetadata(rcComm_t *Conn, rodsEnv myEnv, char *arg0, char *arg1, char *arg2, char *arg3,
	 ...
	 */
	int icommands::modAVUMetadata(rcComm_t *Conn, char *arg0, char *arg1, char *arg2, char *arg3,
	...
	*/
	void icommands::getInput(rcComm_t *Conn, char *cmdToken[], int maxTokens) {
	   int lenstr, i;
	...
	/* handle a command,
	   ...
	   -3 if empty.
	 */
	int icommands::doCommand(rcComm_t *Conn, rodsEnv myEnv, char *cmdToken[]) {
...
//		  usage(cmdToken[1]);
	   ...
	   if (strcmp(cmdToken[0],"adda") == 0) {
		  modAVUMetadata(Conn, "adda", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7], "");
		  return(0);
	   }
	   if (strcmp(cmdToken[0],"add") == 0) {
		  modAVUMetadata(Conn, "add", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7], "");
		  return(0);
	   }
	   if (strcmp(cmdToken[0],"addw") == 0) {
		  int myStat;
		  myStat = modAVUMetadata(Conn, "addw", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7], "");
		  if (myStat > 0) {
	   ...
	   if (strcmp(cmdToken[0],"rmw") == 0) {
		  modAVUMetadata(Conn, "rmw", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7], "");
		  return(0);
	   }
	   if (strcmp(cmdToken[0],"rm") == 0) {
		  modAVUMetadata(Conn, "rm", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7], "");
		  return(0);
	   }
	   if (testMode) {
		 ...
		 modAVUMetadata(Conn, "rmi", cmdToken[1], cmdToken[2],
				cmdToken[3], cmdToken[4], cmdToken[5],
				cmdToken[6], cmdToken[7], "");
		 return(0);
		  }
	   ...
	   if (strcmp(cmdToken[0],"mod") == 0) {
		  modAVUMetadata(Conn, "mod", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
	   ...
	   if (doLs) {
		 ...
		 showDataObj(Conn, cmdToken[2], cmdToken[3], wild);
		 for(int i=0; i<4; i++){
			 printf("cmdToken[%d] = %s \n", i, cmdToken[i]);
		 }
		 return(0);
		 ...
		 showColl(Conn, cmdToken[2], cmdToken[3], wild);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-R")==0 || strcmp(cmdToken[1],"-r")==0) {
		 showResc(Conn, cmdToken[2], cmdToken[3], wild);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-G")==0 || strcmp(cmdToken[1],"-g")==0) {
		 showRescGroup(Conn, cmdToken[2], cmdToken[3], wild);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-u")==0) {
		 showUser(Conn, myEnv, cmdToken[2], cmdToken[3], wild);
		 return(0);
		  }
	   }
	   ...
	   if (strcmp(cmdToken[0],"qu") == 0) {
		  if (strcmp(cmdToken[1],"-d")==0) {
		 queryDataObj(Conn, cmdToken);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-C")==0 || strcmp(cmdToken[1],"-c")==0) {
		 queryCollection(Conn, cmdToken[2], cmdToken[3], cmdToken[4]);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-R")==0 || strcmp(cmdToken[1],"-r")==0) {
		 queryResc(Conn, cmdToken[2], cmdToken[3], cmdToken[4]);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-G")==0 || strcmp(cmdToken[1],"-g")==0) {
		 queryRescGroup(Conn, cmdToken[2], cmdToken[3], cmdToken[4]);
		 return(0);
		  }
		  if (strcmp(cmdToken[1],"-u")==0) {
		 queryUser(Conn, myEnv, cmdToken[2], cmdToken[3], cmdToken[4]);
		 return(0);
		  }
	   }

	   if (strcmp(cmdToken[0],"cp") == 0) {
		  modCopyAVUMetadata(Conn, myEnv, "cp", cmdToken[1], cmdToken[2],
				 cmdToken[3], cmdToken[4], cmdToken[5],
				 cmdToken[6], cmdToken[7]);
		  return(0);
	   }
