  ...
  if test "x$want_postgresql" = "xyes"; then
    ...
    for tmp_path in $tmp_location $POSTGRESQL_LOCATION /usr /usr/local /usr/local/pgsql /opt /opt/local $packages; do
      ...
      if test "x$have_something" = "x"; then
        ...
        have_something=`ls $tmp_path/lib/postgresql*/libpq* 2>/dev/null`
        
        if test "x$have_something" = "x"; then
          AC_MSG_RESULT(no)
          continue
        else
          subdir=`ls -d $tmp_path/lib/postgresql* | head -1 | rev | cut -f 1 -d '/' | rev`
        fi
      fi
      ...
      POSTGRESQL_LDFLAGS="-L$tmp_path/lib/$subdir -lpq"
      ...
      LD_LIBRARY_PATH="$tmp_path/lib/$subdir:$LD_LIBRARY_PATH"
      DYLD_LIBRARY_PATH="$tmp_path/lib/$subdir:$DYLD_LIBRARY_PATH"
