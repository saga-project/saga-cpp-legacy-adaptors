  ...
  : base_cpi (p, info, adaptor, cpi::Noflags)
{
    ...
    if (attr.attribute_exists(saga::attributes::context_type) &&
        "fast_advert_db" != attr.get_attribute(saga::attributes::context_type))
    {
        SAGA_OSSTREAM strm;
        strm << "Can't handle context types others than 'fast_advert_db' "
...
void context_cpi_impl::sync_set_defaults(saga::impl::void_t&)
{
    ...
    if (attr.attribute_exists(saga::attributes::context_type))
    {
        if ("fast_advert_db" != attr.get_attribute(saga::attributes::context_type))
        {
            SAGA_OSSTREAM strm;
            strm << "Can't handle context types others than 'fast_advert_db' "
                 << "(got: " << attr.get_attribute(saga::attributes::context_type) 
