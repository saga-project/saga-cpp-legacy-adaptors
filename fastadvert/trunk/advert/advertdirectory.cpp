...
#define SAGA_ADVERTDIR_DBACTION(name, f)                                      \
    ...
    /* wrap this error in a SAGA adaptor exception */                         \
        SAGA_OSSTREAM strm;                                                   \
        strm << "fast_advert::advertdirectory_cpi_impl::" #name ": "               \
             << e.what();                                                     \
        ...
        detail::get_db_connectstring<instance_data>(adap_ini, p->get_session(), p))
{
    ...
    {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::init: "
    ...
    if (!host.empty() && !detail::host_is_valid(db_.get_dbtype(), host))
    {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::init: "
                "cannot handle advert directory name: " 
             << advert_url.get_url();
    ...
    if (!scheme.empty() && scheme != "fastadvert")
    {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::init: "
                "cannot handle url scheme: " << scheme << ":// - this adaptor only supports fastadvert:// schemes.";
        SAGA_ADAPTOR_THROW(SAGA_OSSTREAM_GETSTRING(strm), saga::adaptors::AdaptorDeclined);
    }
...
advertdirectory_cpi_impl::sync_get_attribute(std::string& ret, std::string key)
{
    ...
    if (!is_vector) {
        ...
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::sync_get_attribute: "
    ...
    std::string key)
{
    ...
    if (result) {
        if (is_vector) {
            ...
            SAGA_OSSTREAM strm;
            strm << "fast_advert::advertdirectory_cpi_impl::sync_get_vector_attribute: "
...
advertdirectory_cpi_impl::sync_get_cwd(saga::url& url)
{
    ...
    url = result;
    url.set_path(p.parent_path().string());
}
...
advertdirectory_cpi_impl::sync_remove(saga::impl::void_t&, int flags)
{
    ...
    if (!(flags & saga::advert::Recursive)) {
        SAGA_ADAPTOR_THROW(
            "fast_advert::advertdirectory_cpi_impl::sync_remove: "
...
namespace 
{
        ...
        std::vector<std::string>& ret)
    {
             ...
             it != end; ++it)
        {
            ...
            // this should not happen!
                SAGA_OSSTREAM strm;
                strm << "fast_advert::advertdirectory_cpi_impl::find_recursive: "
        ...
        std::vector<std::string>& ret)
    {
             ...
             it != end; ++it)
        {
            ...
            // this should not happen!
                SAGA_OSSTREAM strm;
                strm << "fast_advert::advertdirectory_cpi_impl::list_recursive: "
                        "Database inconsistency detected: " << entry;
                SAGA_ADAPTOR_THROW_NO_CONTEXT(SAGA_OSSTREAM_GETSTRING(strm), 
                    saga::NoSuccess);
            }
    ...
    std::string pattern, int flags)
{
    ...
    std::string basename;
    {
        ...
        adname = detail::get_filepath(data->location_);
        basename = data->location_.get_url();
    }
    ...
    if (flags & saga::advert::Dereference) {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::sync_list: "
    ...
    std::string pattern, int flags)
{
    ...
    if (flags & saga::advert::Dereference) {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::sync_find: "
                "Dereferencing links is not supported.";
        SAGA_ADAPTOR_THROW(SAGA_OSSTREAM_GETSTRING(strm), 
            saga::NotImplemented);
    }
...
advertdirectory_cpi_impl::sync_get_entry(saga::url& entry, std::size_t num)
{
    ...
    if (num < 0 || num >= entries.size()) {
        SAGA_OSSTREAM strm;
        strm << "fast_advert::advertdirectory_cpi_impl::sync_get_entry: "
...
advertdirectory_cpi_impl::sync_change_dir(saga::impl::void_t&, saga::url dir)
{
    ...
    // check permissions (throws on mismatch)
    check_permissions(saga::advert::Write, "change_dir", adname.get_url());
    ...
    int flags)
{
    ...
    if (result) {
        ...
        if (flags & saga::advert::Exclusive) {
            SAGA_ADAPTOR_THROW(
                "fast_advert::advertdirectory_cpi_impl::sync_make_dir: "
