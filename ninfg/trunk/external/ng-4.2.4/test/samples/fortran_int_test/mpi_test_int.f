C $RCSfile: mpi_test_int.f,v $ $Revision: 1.1 $ $Date: 2007/05/29 06:47:20 $
C $AIST_Release: 4.2.4 $
C $AIST_Copyright:
...
C  $
        ...
        subroutine mpi_test_int(n,scalarIn,scalarOut,arrayIn,arrayOut)
        implicit none
        include 'mpif.h'
...
C for debug
        ...
        if (my_rank .ne. 0) then
           call MPI_SEND(arrayOut(my_rank+1), 1, MPI_INTEGER, 
...
C for debug
           ...
           do i = 1, n-1
             buffer(i) = 0
             call MPI_Recv(buffer(i), 1, MPI_INTEGER, 
