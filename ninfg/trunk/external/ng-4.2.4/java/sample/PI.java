...
public class PI implements Runnable {
	...
	public PI(GrpcClient context, String hostName, int numHosts, int number) {
	...
	public static void main (String args[]) {
		...
		try {
			...
			/* call function */
			PI[] piArray = new PI[hostName.length];
			Thread[] piThread = new Thread[hostName.length];
			for (int i = 0; i < hostName.length; i++) {
				piArray[i] = new PI(client, hostName[i], hostName.length, i);
