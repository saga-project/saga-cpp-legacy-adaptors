#ifndef NG_OS_IRIX
static const char rcsid[] = "$RCSfile: ngexSignalManager.c,v $";
#endif /* NG_OS_IRIX */
/*
 * $AIST_Release: 4.2.4 $
 * $AIST_Copyright:
...
ngexiSignalManagerInitialize(ngLog_t *log, int *error)
{
	...
	ngLogPrintf(log, NG_LOG_CATEGORY_NINFG_PURE, NG_LOG_LEVEL_ERROR, NULL,
	    "%s: Signal Manager is already initialized.\n", fName);
	goto error;
    }
...
ngexlTemporaryFileRegister(char *fileName, ngLog_t *log, int *error)
{
	...
	ngexlSignalManager.ngsm_tmpFile_tail->ngtf_next
	    = (struct ngexlTemporaryFile_s *)tmpFile;
	ngexlSignalManager.ngsm_tmpFile_tail = tmpFile;
    }
...
ngexlTemporaryFileAllocate(char *fileName, ngLog_t *log, int *error)
{
    ngexlTemporaryFile_t *tmpFile;
 ...
 */
static int
ngexlTemporaryFileFree(ngexlTemporaryFile_t *tmpFile, ngLog_t *log, int *error)
{
    /* Check the arguments */
    assert(tmpFile != NULL);
...
ngexlSignalManagerMutexUnlock(ngLog_t *log, int *error)
{
	...
	ngLogPrintf(log, NG_LOG_CATEGORY_NINFG_PURE, NG_LOG_LEVEL_FATAL,
	    NULL, "%s: pthread_mutex_unlock failed: %s.\n",
	    fName, strerror(result));
        /* Failed */
        return 0;
    }
