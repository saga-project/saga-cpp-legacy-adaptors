/* 
 * $RCSfile: ngGenerator.h,v $ $Revision: 1.3 $ $Date: 2008/03/05 10:50:19 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
#define NGG_LIST_NEXT(lp) ((lp)->l_next)
#define NGG_LIST_ITEM(lp) ((lp)->l_item)
#define FOR_ITEMS_IN_LIST(lp, x) \
  if(x != NULL) for(lp = EXPR_LIST(x); lp != NULL ; lp = NGG_LIST_NEXT(lp))

#define EXPR_ARG1(x) NGG_LIST_ITEM(EXPR_LIST(x))
#define EXPR_ARG2(x) NGG_LIST_ITEM(NGG_LIST_NEXT(EXPR_LIST(x)))
#define EXPR_ARG3(x) NGG_LIST_ITEM(NGG_LIST_NEXT(NGG_LIST_NEXT(EXPR_LIST(x))))
#define EXPR_ARG4(x) NGG_LIST_ITEM(NGG_LIST_NEXT(NGG_LIST_NEXT(NGG_LIST_NEXT(EXPR_LIST(x)))))
