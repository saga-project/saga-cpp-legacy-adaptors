/*
 * $RCSfile: nggenStub.c,v $ $Revision: 1.11 $ $Date: 2008/03/05 10:50:19 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: nggenStub.c,v $ $Revision: 1.11 $ $Date: 2008/03/05 10:50:19 $")
...
)
{
    ...
    /* temporary NRF generation, fix it before release. */
    fprintf(fp,"\n# temporary NRF dummy target.\nNRF_DUMMY = nrf_dummy");


    fprintf(fp,"\n\nall: $(NG_STUB_PROGRAM) ");
    fprintf(fp,"$(NG_INF_FILES) $(NRF_DUMMY)");
    fprintf(fp,"\n\n");
    ...
    for ( curr = stubs_head; curr != NULL; curr = curr->next_stub ) {
        ...
        fprintf(fp, "_stub_%s.o: _stub_%s.c\n\t$(NG_COMPILER) "
            "$(CFLAGS) $(NG_CFLAGS) $(NG_USER_CFLAGS) $(NG_CPPFLAGS) -c _stub_%s.c\n\n",
        ...
        if (curr->backend == NG_BACKEND_MPI ||
            ...
            fprintf(fp, "_stub_%s-info.o: _stub_%s.c\n"
				"\t$(CC) $(NG_CFLAGS) $(CFLAGS) $(NG_USER_CFLAGS) $(NG_CPPFLAGS) "
			"-D%s "
	...
	fprintf(fp, "\t$(NG_DIR)/bin/ng_gen_nrf $(NG_STUB_PROGRAM)\n");
...
)
{
    ...
    if ( ep->body_expr ) { /* generate body                      */
        ...
        FOR_ITEMS_IN_LIST(lp, EXPR_ARG3(ep->body_expr)) {
            tmp = SYM_NAME(EXPR_SYM(NGG_LIST_ITEM(lp)));
	    ...
	    language != NULL &&
            strcasecmp(language, FORTRAN) == 0){
          FOR_ITEMS_IN_LIST(lp,EXPR_ARG3(ep->body_expr)){
            char * tmp = SYM_NAME(EXPR_SYM(NGG_LIST_ITEM(lp)));
...
)
{
    ...
    fprintf(fp, "#include \"grpc_executable.h\"\n\n");
