/*
 * $RCSfile: nggenCompile.c,v $ $Revision: 1.5 $ $Date: 2008/02/07 10:26:16 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: nggenCompile.c,v $ $Revision: 1.5 $ $Date: 2008/02/07 10:26:16 $")
		      ...
		      struct stub_gen_entry * ep, int method_index)
{
	...
	n_param += nggen_cpl_count_params(NGG_LIST_ITEM(lp));
    }
    /* init params */
...
)
{
    ...
    FOR_ITEMS_IN_LIST(lp, EXPR_ARG2(param)) {
        index++;
        nggen_cpl_param(++dp, NGG_LIST_ITEM(lp));
    }
    return index;
}
...
)
{
    ...
    switch ( EXPR_CODE(decl) ) {
        ...
        default: /* !!! NOTE: other declaration is not implemented yet !!! */
            nggen_error_at_node(decl, "unknown declarator");
            rc = 0;
            break;
    }
    return(rc);
...
)
{
    ...
    switch ( EXPR_CODE(param) ) {
        ...
        case POINTER_REF: /* POINTER_REF is same as size 1 array */
            ...
            break;
        default: /* !!! NOTE: other declaration is not implemented yet !!! */
            nggen_error_at_node(param, "unknown declarator");
            ndim = 0;
            break;
    }
...
)
{
    ...
    if ( param != NULL ) {
        if ( EXPR_CODE(param) == CALLBACK_FUNC ) {
            ...
            FOR_ITEMS_IN_LIST(lp, EXPR_ARG2(param)) {
                index++;
                nggen_cpl_count_dim(++dp, 0, NGG_LIST_ITEM(lp));
            }
            return index;
        } else {
