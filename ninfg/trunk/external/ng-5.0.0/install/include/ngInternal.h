/* 
 * $RCSfile: ngInternal.h,v $ $Revision: 1.30 $ $Date: 2008/03/28 09:26:28 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
/**
 * Information of Log level
 */
typedef struct ngLogLevelInformation_s {
    ngLogLevel_t nglli_level;
    ngLogLevel_t nglli_ninfgProtocol;
    ngLogLevel_t nglli_ninfgInternal;
    ngLogLevel_t nglli_grpc;
} ngLogLevelInformation_t;
 ...
 */
#if 0
/* If you want to set TCP_NODELAY to true, set to TRUE.
 * This macro sets TCP_NODELAY for all connections in
 * the Client and Executables connecting to that Client.
 *
 * Although you set this macro, you can set TCP_NODELAY to false
 * by client configuration file.
 */
#define NGI_SET_TCP_NODELAY
#endif

/* Default value of TCP_NODELAY in Remote Machine information */
#ifdef NGI_SET_TCP_NODELAY
#define NGI_TCP_NODELAY_DEFAULT 1
#else /* NGI_SET_TCP_NODELAY */
#define NGI_TCP_NODELAY_DEFAULT 0
#endif /* NGI_SET_TCP_NODELAY */
...
#define NGI_PROTOCOL_VERSION_MAJOR	5
#define NGI_PROTOCOL_VERSION_MINOR	0
...
#define NGI_PROTOCOL_VERSION_MINIMUM	0x05000000
...
/* Simple Auth Number */
#define NGI_PROTOCOL_SIMPLE_AUTH_NUMBER_UNDEFINED 0
#define NGI_PROTOCOL_SIMPLE_AUTH_NUMBER_MIN 1
#define NGI_PROTOCOL_SIMPLE_AUTH_NUMBER_MAX INT_MAX
...
#define NGI_PROTOCOL_NEGOTIATION_OPTION_SIZE_MAX (3 + 3) /* Zlib + Division */

/**
 * Identification number
 */
/* Connect Count */
#define NGI_CONNECT_COUNT_MIN   0		/* Minimum value */
#define NGI_CONNECT_COUNT_MAX   0x7fffffff	/* Minimum value */
#define NGI_CONNECT_COUNT_UNDEFINED (-1)	/* Undefined value */
...
#define NGI_PROTOCOL_REQUEST_CODE_CONNECTION_CLOSE			0x41
...
typedef struct ngiStreamManager_s {
    struct ngiStreamManagerTypeInformation_s *ngsm_typeInformation;
...
#define ngiStreamManagerDestroyWriteData(sMng, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_destroyWriteData(sMng, log, error))
#define ngiStreamManagerDestroyReadData(sMng, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_destroyReadData(sMng, log, error))
#define ngiStreamManagerGetWritableBuffer(sMng, buf, nBytesRequired, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_getWritableBuffer(sMng, buf, nBytesRequired, nBytes, log, error))
#define ngiStreamManagerWriteBuffer(sMng, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_writeBuffer(sMng, nBytes, log, error))
#define ngiStreamManagerGetReadableBuffer(sMng, buf, nBytesRequired, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_getReadableBuffer(sMng, buf, nBytesRequired, nBytes, log, error))
#define ngiStreamManagerReadBuffer(sMng, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_readBuffer(sMng, nBytes, log, error))
#define ngiStreamManagerWriteDirectly(sMng, buf, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_writeDirectly(sMng, buf, nBytes, log, error))
#define ngiStreamManagerGetBytesOfReadableData(sMng, isTooLarge, nBytes, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_getBytesOfReadableData(sMng, isTooLarge, nBytes, log, error))
...
typedef struct ngiCommunication_s {
    ngCommLog_t               *ngc_commLog;     /* Communication log */
    unsigned short             ngc_portNo;      /* Number of the port */
    ngiIOhandle_t             *ngc_ioHandle;    /* Handle of I/O */
    ngiIOhandleAcceptResult_t *ngc_acceptResult;/* result of accept */
} ngiCommunication_t;

typedef struct ngiAddress_s {
    ngiIOhandleSocketType_t nga_type;
    char *nga_hostname;
    int   nga_port;
    char *nga_path;
} ngiAddress_t;
...
typedef struct ngiProtocolAttribute_s {
    ...
    int ngpa_simpleAuthNumber;	/* Simple Auth Number */
    ...
    int ngpa_keepConnect;	/* Keep connection */
} ngiProtocolAttribute_t;

/**
 * Attribute of Transfer.
 */
typedef struct ngiProtocolTransferAttribute_s {
    int  ngpta_valid;
    long ngpta_contextID;    /* Context ID */
    long ngpta_executableID; /* Executable ID */
    long ngpta_sessionID;    /* Session ID */
...
typedef struct ngiProtocolNegotiationFromExecutable_s {
    ...
    long	ngpnfe_simpleAuthNumber; /* Simple Auth Number */
    long	ngpnfe_connectCount;	/* Connect count */
    long	ngpnfe_contextID;	/* Context ID */
    long	ngpnfe_jobID;		/* Job ID */
    long	ngpnfe_executableID;	/* Executable ID */
...
typedef struct ngiProtocolNegotiationFromClient_s {
    ...
    long	ngpnfc_simpleAuthNumber; /* Simple Auth Number */
    long	ngpnfc_connectCount;	/* Connect count */
    long	ngpnfc_contextID;	/* Context ID */
    long	ngpnfc_jobID;		/* Job ID */
...
typedef struct ngiProtocol_s {
    ...
    ngiProtocolTransferAttribute_t ngp_transferAttr; /* Transfer attribute. */
    ...
    ngiMutex_t          ngp_sendMutex;          /* Mutex for send */

    ngiCommunication_t	*ngp_communication;	/* Communication */
    ngiEvent_t		*ngp_event;		/* ngiEvent for I/O handle */

    /* Sequence number of packet */
    int	ngp_sequenceNo;				/* Request/Reply */
    int ngp_notifySeqNo;			/* Notify */

    int ngp_connectCount;			/* Connect count */
    int ngp_doClose;				/* Close connection */
...
#define ngiSendRequestConnectionClose(protocol, log, error) \
    (protocol)->ngp_SendRequestCommon( \
	protocol, NGI_PROTOCOL_REQUEST_CODE_CONNECTION_CLOSE, \
	NGI_SESSION_ID_UNDEFINED, log, error)
...
#define ngiSendReplyConnectionClose(protocol, result, log, error) \
    (protocol)->ngp_SendReplyCommon( \
	protocol, NGI_PROTOCOL_REQUEST_CODE_CONNECTION_CLOSE, \
	NGI_SESSION_ID_UNDEFINED, result, log, error)
...
ngiProtocol_t *ngiProtocolConstruct(
    ngiProtocolAttribute_t *, ngiCommunication_t *, ngiEvent_t *,
    ngRemoteMethodInformation_t *(*)(ngiProtocol_t *, ngLog_t *, int*),
    ngLog_t *, int *);
...
int ngiProtocolSetSimpleAuthNumber(ngiProtocol_t *, int, ngLog_t *, int *);
int ngiProtocolSetJobID(ngiProtocol_t *, int, ngLog_t *, int *);
int ngiProtocolSetExecutableID(ngiProtocol_t *, int, ngLog_t *, int *);
int ngiProtocolSetKeepConnect(ngiProtocol_t *, int, ngLog_t *, int *);
int ngiProtocolAttributeInitialize(ngiProtocolAttribute_t *, ngLog_t *, int *);
int ngiProtocolTransferAttributeInitialize(ngiProtocol_t *, ngLog_t *, int *);
int ngiProtocolTransferAttributeSet(ngiProtocol_t *, ngLog_t *, int *);
...
ngiProtocolSetCommunication(
    ngiProtocol_t *, ngiCommunication_t *, ngLog_t *, int *);
int ngiProtocolConnected(ngiProtocol_t *, ngLog_t *l, int *);
int ngiProtocolSetConnectionClose(ngiProtocol_t *, int, ngLog_t *, int *);
int ngiProtocolGetConnectionClose(ngiProtocol_t *, int *, ngLog_t *, int *);
int ngiProtocolReleaseConnectionClose(ngiProtocol_t *, ngLog_t *, int *);
...
int ngiProtocolGetSessionInformationFromXML(
    ngiProtocol_t *, char *, ngSessionInformationExecutable_t *,
    ngSessionInformationExecutable_t *,
    int *, ngCompressionInformation_t *, int, ngLog_t *, int *);
int ngiProtocolGetXMLstringFromSessionInformation(ngiProtocol_t *,
    ngSessionInformationExecutable_t *, ngSessionInformationExecutable_t *,
    int, ngCompressionInformation_t *, int, char *, size_t, ngLog_t *, int *);
...
ngiStreamManager_t *ngiFileStreamManagerConstruct(char *, ngiEvent_t *, ngLog_t *, int *);
...
int ngiFileStreamManagerInitialize(ngiFileStreamManager_t *, char *, ngiEvent_t *, ngLog_t *, int *);
...
ngiCommunication_t *ngiCommunicationConstructServer(
    ngiEvent_t *, int, ngLog_t *, int *);
ngiCommunication_t *ngiCommunicationConstructAccept(
    ngiEvent_t *, ngiCommunication_t *, ngLog_t *, int *);
...
char *ngiCommunicationGetContactString(
    ngiCommunication_t *, const char *, ngLog_t *, int *);

int ngiCommunicationLogRegister(
    ngiCommunication_t *, ngCommLog_t *, ngLog_t *, int *);
...
int ngiCommunicationSetTcpNodelay(
    ngiCommunication_t *, int, ngLog_t *, int *);
int ngiCommunicationPeerIsLocalhost(
    ngiCommunication_t *, int *, ngLog_t *, int *);

int ngiCommunicationParseAddress(
    const char *, ngiAddress_t*, ngLog_t *, int *);

int ngiAddressInitialize(ngiAddress_t *, ngLog_t *, int *);
int ngiAddressFinalize(ngiAddress_t *, ngLog_t *, int *);

/* Information of Log level */
int ngiLogInformationInitialize(ngLogInformation_t *, ngLog_t *, int *);
int ngiLogInformationFinalize(ngLogInformation_t *, ngLog_t *, int *);
void ngiLogInformationInitializeMember(ngLogInformation_t *log);
int ngiLogInformationCopy(
    ngLogInformation_t *, ngLogInformation_t *, ngLog_t * , int *);
int ngiLogInformationSetDefault(
    ngLogInformation_t *, ngLog_t *, int*);

int ngiLogLevelInformationInitialize(
    ngLogLevelInformation_t *, ngLog_t *, int *);
int ngiLogLevelInformationFinalize(
    ngLogLevelInformation_t *, ngLog_t *, int *);
void ngiLogLevelInformationInitializeMember(ngLogLevelInformation_t *);
int ngiLogLevelInformationCopy(
    ngLogLevelInformation_t *, ngLogLevelInformation_t *, ngLog_t *, int *);
int ngiLogLevelInformationSetDefault(
    ngLogLevelInformation_t *, ngLog_t *, int *);

ngLog_t *ngiLogConstructFromConfig(
    ngLogInformation_t *, ngLogLevelInformation_t *, const char *,
    int, ngLog_t *, int *);

/* For Communication Proxy*/
char *ngiCommunicationProxyEncode(const char *, ngLog_t *, int *);
char *ngiCommunicationProxyDecode(const char *, ngLog_t *, int *);
