...
# $RCSfile: gt4invokeserver.py,v $ $Revision: 1.12 $ $Date: 2008/02/08 01:57:46 $
# $AIST_Release: 5.0.0 $
# $AIST_Copyright:
...
# True: supported option, False: unsupported option.
option_table = {}
option_table['hostname'] = True
option_table['port'] = True
option_table['jobmanager'] = True
option_table['subject'] = True
option_table['client_name'] = True
option_table['executable_path'] = True
option_table['backend'] = True
option_table['count'] = True
option_table['staging'] = True
option_table['argument'] = True
option_table['work_directory'] = True
option_table['redirect_enable'] = True
option_table['stdout_file'] = True
option_table['stderr_file'] = True
option_table['environment'] = True
option_table['tmp_dir'] = True
option_table['status_polling'] = True
option_table['refresh_credential'] = True
option_table['max_time'] = True
option_table['max_wall_time'] = True
option_table['max_cpu_time'] = True
option_table['queue_name'] = True
option_table['project'] = True
option_table['host_count'] = True
option_table['min_memory'] = True
option_table['max_memory'] = True
option_table['rsl_extensions'] = True
option_table['delegate_full_proxy'] = True
option_table['protocol'] = True
option_table['gsiftp_port'] = True
option_table['staging_source_subject'] = True
option_table['staging_destination_subject'] = True
option_table['deletion_subject'] = True
option_table['auth_number'] = True
option_table['communication_proxy_staging'] = True
option_table['communication_proxy_path'] = True
...
class Job(object):
	...
	def __init__(self, req_id, attrs):
		...
		self.auth_file_name = None
		self.remote_auth_file = None
		self.commProxyFileName = None
		self.remoteCommProxyFileName = None
		self.status = None
	...
	def auth_file(self):
		self.auth_number = self.attrs.get('auth_number')
		if not self.auth_number:
			return
		fd, self.auth_file_name = tempfile.mkstemp('', 'invoke_server_auth.')
		os.write(fd, self.auth_number);
		os.close(fd);
		self.remote_auth_file = '${GLOBUS_SCRATCH_DIR}/auth.' + self.job_id[5:]

	def commProxyFile(self):
		self.commProxyFileName = self.attrs.get('communication_proxy_path')
		if not self.commProxyFileName:
			return
		if self.attrs.get('communication_proxy_staging') == 'true':
			self.remoteCommProxyFileName = '${GLOBUS_SCRATCH_DIR}/proxy.' + self.job_id[5:]
		else:
			self.remoteCommProxyFileName = self.commProxyFileName
	...
	def build_xmlrsl(self):
		...
		if self.remote_auth_file:
			rsl.append(['argument', '--authNumberFile=' + self.remote_auth_file])
		if self.remoteCommProxyFileName:
			rsl.append(['argument', '--communicationProxyPath=' + self.remoteCommProxyFileName])
		...
		fileStageInFlag = False
		fileStageIn = ['fileStageIn']
		if staging == 'true':
			fileStageInFlag = True
		...
		if self.auth_file_name:
			fileStageInFlag = True
			fileStageIn.append(['transfer', ['sourceUrl', 'gsiftp://' + self.attrs['client_name'] + ':' + gsiftp_port + self.auth_file_name],
				['destinationUrl', 'file:///' + self.remote_auth_file] ])
		if self.attrs.get('communication_proxy_staging') == 'true':
			fileStageInFlag = True
			fileStageIn.append(['transfer', ['sourceUrl', 'gsiftp://' + self.attrs['client_name'] + ':' + gsiftp_port + self.commProxyFileName],
				['destinationUrl', 'file:///' + self.remoteCommProxyFileName] ])
		if fileStageInFlag == True:
			...
			if self.auth_file_name:
				fileCleanUp.append(['deletion', ['file', 'file:///' + self.remote_auth_file]])
			if self.remoteCommProxyFileName:
				fileCleanUp.append(['deletion', ['file', 'file:///' + self.remoteCommProxyFileName]])
...
class GT4InvokeServer(object):
	...
	def default_handler(self, line):
		...
		elif token == 'QUERY_FEATURES':
			self.cmd_query_features()
		elif token == 'EXIT':
			self.cmd_exit()
		else:
		...
		def _handler(line):
			...
			if line != 'JOB_CREATE_END':
				...
				if not key:
					...
					self.handler = self.default_handler
					return
				if not option_table.has_key(key):
					logging.getLogger().warning('Unknown option "%s"' % key)
					return
				elif not option_table[key]:
					logging.getLogger().warning('Unsupported option "%s"' % key)
					return
				if key == 'argument':
			...
			if req_attrs.get('communication_proxy_staging') == 'true':
				if not req_attrs.get('communication_proxy_path'):
					Responder.send('F communication_proxy_path is not specified\r\n')
					self.handler = self.default_handler
					return
	...
	def cmd_query_features(self):
		Responder.send( \
			'SM\r\n' + \
			'protocol_version 2.0\r\n' + \
			'feature STAGING_AUTH_NUMBER\r\n' + \
			'feature STAGING_COMMUNICATION_PROXY\r\n' + \
			'request JOB_CREATE\r\n' + \
			'request JOB_STATUS\r\n' + \
			'request JOB_DESTROY\r\n' + \
