/* 
 * $RCSfile: ngInvokeServer.h,v $ $Revision: 1.7 $ $Date: 2008/02/26 06:32:39 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
#define NGISI_GASS_SCHEME "https"
#define NGISI_GASS_OPTION \
...
typedef struct ngisiContext_s {
    ...
    globus_cond_t        ngisc_jobCond;
    int                  ngisc_jobCondInitialized;
    ...
    /* for GASS server */
    int			 ngisc_gassInitialized;
    char		*ngisc_gassURL;
    globus_gass_transfer_listener_t ngisc_gassListener;
} ngisiContext_t;
...
int ngisiContextJobListNotify(ngisiContext_t *, int, int *);
int ngisiContextJobListWait(ngisiContext_t *, int, int *);
...
int ngisiCondBroadcast(ngisiContext_t *, globus_cond_t *, int *);
