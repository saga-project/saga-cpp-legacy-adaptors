/*
 * $RCSfile: ngisJob.c,v $ $Revision: 1.9 $ $Date: 2008/02/26 06:32:39 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngisJob.c,v $ $Revision: 1.9 $ $Date: 2008/02/26 06:32:39 $")
    ...
    int *error)
{
    ...
    result = ngisiContextJobListNotify(context, listLocked, error);
    if (result == 0) {
        ngisiLogPrintf(context, NGISI_LOG_LEVEL_ERROR, fName,
...
{
    ngisiJob_t *job;
    int result, doDestroy, listLocked;
    ...
    /* Treat the no JOB_DESTROY received job. */
    job = context->ngisc_job_head;
    while (job != NULL) {

        doDestroy = 0;
        if (job->ngisj_destructableProtocol == 0) {
            doDestroy = 1;
            result = ngisiJobProtocolDestroy(
                context, job->ngisj_jobID, error);
            if (result == 0) {
                ngislLogPrintfJob(job, NGISI_LOG_LEVEL_WARNING, fName,
                    "cancel the job failed.\n");
                /* Not return. */
            }
        }
    ...
    ngisiLogPrintf(context, NGISI_LOG_LEVEL_DEBUG, fName,
        "Start waiting the remaining jobs.\n");

    while (context->ngisc_job_head != NULL) {
        result = ngisiContextJobListWait(context, listLocked, error);
        if (result == 0) {
            ngisiLogPrintf(context, NGISI_LOG_LEVEL_ERROR, fName,
                "Notify the Job list failed.\n");
            goto error;
    ...
    ngisiLogPrintf(context, NGISI_LOG_LEVEL_DEBUG, fName,
        "Finish waiting the remaining jobs.\n");
    ...
    int *error)
{
    ...
    if (jobAttr->ngisja_redirect != 0) {
        if (context->ngisc_gassURL == NULL) {
            ngislLogPrintfJob(job, NGISI_LOG_LEVEL_ERROR, fName,
                "redirect stdout/stderr enable, but no GASS URL.\n");
            goto error;
        }
...
ngislJobCancel(
    ...
    int *error)
{
    static const char fName[] = "ngislJobCancel";
    int result;
    ...
    if ((job->ngisj_status == NGISI_JOB_STATUS_DONE) ||
        (job->ngisj_status == NGISI_JOB_STATUS_FAILED)) {
        ngislLogPrintfJob(job, NGISI_LOG_LEVEL_DEBUG, fName,
        ...
        ngislLogPrintfJob(job, NGISI_LOG_LEVEL_DEBUG, fName,
            "Cancel the job %s.\n", job->ngisj_jobContact);
        result = globus_gram_client_job_cancel(job->ngisj_jobContact);
        if (result != GLOBUS_SUCCESS) {
            ngislLogPrintfJob(job, NGISI_LOG_LEVEL_DEBUG, fName,
                "%s failed by %d: %s.\n",
    ...
    int *error)
{
    ...
    char *rslBuf, *tmp, *env;
    char *resultPtr, *prefix, *separator;
    char workingDir[NGISI_PATH_MAX];
    ...
    *rsl = NULL;
    ...
    bufferNbytes += ((context->ngisc_gassURL != NULL) ?
        strlen(context->ngisc_gassURL) : 0);
    ...
    /* executable */
    if ((jobAttr->ngisja_staging != 0) && (context->ngisc_gassURL == 0)) {
        ngislLogPrintfJob(job, NGISI_LOG_LEVEL_ERROR, fName,
    ...
    if (jobAttr->ngisja_redirect != 0) {
        ...
        if (jobAttr->ngisja_stdoutFile != NULL) {
            if (jobAttr->ngisja_stdoutFile[0] == '/') {
                prefix = "";
                separator = "";
            } else {
                prefix = workingDir;
            ...
            length += snprintf(&rslBuf[length], bufferNbytes - length,
                "(stdout = $(NG_GASS_URL) # %s%s%s)",
                prefix, separator, jobAttr->ngisja_stdoutFile);
            if (length >= (bufferNbytes - 1)) {
                goto overflow;
            }
        }
        ...
        if (jobAttr->ngisja_stderrFile != NULL) {
            if (jobAttr->ngisja_stderrFile[0] == '/') {
                prefix = "";
                separator = "";
            } else {
                prefix = workingDir;
            ...
            length += snprintf(&rslBuf[length], bufferNbytes - length,
                "(stderr = $(NG_GASS_URL) # %s%s%s)",
                prefix, separator, jobAttr->ngisja_stderrFile);
            if (length >= (bufferNbytes - 1)) {
                goto overflow;
            }
        }
    ...
    int *error)
{
    ...
    result = ngislJobCancel(context, job, error);
    if (result == 0) {
        ngislLogPrintfJob(job, NGISI_LOG_LEVEL_ERROR, fName,
            "cancel the job failed.\n");
        goto error;
