/*
 * $RCSfile: ngisJobAttribute.c,v $ $Revision: 1.4 $ $Date: 2008/03/03 09:11:21 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngisJobAttribute.c,v $ $Revision: 1.4 $ $Date: 2008/03/03 09:11:21 $")
    ...
    ngisOptionContainer_t *opts)
{
    ...
    /* Set value */
    r = ngisAttrSetString(&new->ngja_hostname,         opts, "hostname",           1) &&
        ngisAttrSetUshort(&new->ngja_port,             opts, "port",               1) &&
        ngisAttrSetString(&new->ngja_jobManager,       opts, "jobmanager",         0) &&
        ngisAttrSetString(&new->ngja_clientHostname,   opts, "client_name",        1) &&
        ngisAttrSetString(&new->ngja_executablePath,   opts, "executable_path",    1) &&
        ngisAttrSetBackend(&new->ngja_jobBackend,      opts, "backend",            1) &&
        ngisAttrSetStringList(&new->ngja_arguments,    opts, "argument"             ) &&
        ngisAttrSetString(&new->ngja_workDirectory,    opts, "work_directory",     0) &&
        ngisAttrSetStringList(&new->ngja_environments, opts, "environment"          ) &&
        ngisAttrSetUint(&new->ngja_statusPolling,      opts, "status_polling",     1) &&
        ngisAttrSetUint(&new->ngja_refreshCredential,  opts, "refresh_credential", 1) &&
        ngisAttrSetUint(&new->ngja_count,              opts, "count",              1) &&
        ngisAttrSetBool(&new->ngja_staging,            opts, "staging",            1) &&
        ngisAttrSetBool(&new->ngja_redirectEnable,     opts, "redirect_enable",    1) &&
        ngisAttrSetString(&new->ngja_stdoutFile,       opts, "stdout_file",        0) &&
        ngisAttrSetString(&new->ngja_stderrFile,       opts, "stderr_file",        0) &&
        ngisAttrSetInt(&new->ngja_maxTime,             opts, "max_time",           0) &&
        ngisAttrSetInt(&new->ngja_maxWallTime,         opts, "max_wall_time",      0) &&
        ngisAttrSetInt(&new->ngja_maxCpuTime,          opts, "max_cpu_time",       0) &&
        ngisAttrSetString(&new->ngja_queueName,        opts, "queue_name",         0) &&
        ngisAttrSetString(&new->ngja_project,          opts, "project",            0) &&
        ngisAttrSetInt(&new->ngja_hostCount,           opts, "host_count",         0) &&
        ngisAttrSetInt(&new->ngja_minMemory,           opts, "min_memory",         0) &&
        ngisAttrSetInt(&new->ngja_maxMemory,           opts, "max_memory",         0) &&
        ngisAttrSetString(&new->ngja_tmpDir,           opts, "tmp_dir",            0) &&
        ngisAttrSetString(&new->ngja_gassURL,          opts, "gass_url",           0) &&
        ngisAttrSetStringList(&new->ngja_rslExtensions,opts, "rsl_extensions"       ) &&
        ngisAttrSetUint(&new->ngja_authNumber,         opts, "auth_number",        0) &&
        ngisAttrSetBool(&new->ngja_commProxyStaging,   opts, "communication_proxy_staging", 0) &&
        ngisAttrSetString(&new->ngja_commProxyPath,    opts, "communication_proxy_path",    0);
    if (r == 0) {
        goto error;
    }
    ...
    if (new->ngja_commProxyStaging) {
        if (new->ngja_commProxyPath == NULL) {
            ngisErrorPrint(NULL, fName, "\"communication_proxy_staging\" is true, "
                                       "but \"communication_proxy_path\" doesn't exist.\n");
            goto error;
        }
    }
    ...
    ngisJobAttributes_t *attr)
{
    ...
    attr->ngja_authNumber             = 0;
    attr->ngja_commProxyStaging       = 0; /* False */
    attr->ngja_commProxyPath          = NULL;
    ...
    ngisJobAttributes_t *attr)
{
    ...
    NGIS_NULL_CHECK_AND_FREE(attr->ngja_commProxyPath);
