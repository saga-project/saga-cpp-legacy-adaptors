/*
 * $RCSfile: ngisSSHprepare.c,v $ $Revision: 1.4 $ $Date: 2008/03/28 03:52:30 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngisSSHprepare.c,v $ $Revision: 1.4 $ $Date: 2008/03/28 03:52:30 $")
...
static int ngislSSHjobStagingSimpleAuthNumber(ngisSSHjob_t *);
    ...
    ngisSSHjob_t *job)
{
    ...
    if ((job->ngsj_attributes->ngsja_staging != 0)                 ||
        ...
        (job->ngsj_attributes->ngsja_authNumber > 0)               ||
    ...
    /* Staging auth file*/
    if (job->ngsj_attributes->ngsja_authNumber > 0) {
        result = ngisSSHjobFunctionPush(job, ngislSSHjobStagingSimpleAuthNumber);
        if (result == 0) {
            ngisErrorPrint(log, fName,
                "Can't register function for staging "
                " simple auth number.\n");
            return 0;
        }
    }
    ...
    ngisSSHjob_t *job)
{
    ...
    if (job->ngsj_attributes->ngsja_authNumber > 0) {
        job->ngsj_remoteAuthNumberPath = 
            ngisStrdupPrintf("%s/auth_number", job->ngsj_remoteTempdir);
        if (job->ngsj_remoteAuthNumberPath == NULL) {
            ngisErrorPrint(log, fName,
                "Can't create string of tempfile for simple auth number.\n");
            return 0;
        }
    }
...
static int
ngislSSHjobStagingSimpleAuthNumber(
    ngisSSHjob_t *job)
{
    ...
    char *string2 = NULL;
    int ret = 0;
    static const char fName[] = "ngislSSHjobStagingSimpleAuthNumber";

    NGIS_ASSERT(job != NULL);

    log = job->ngsj_log;

    string = ngisStrdupPrintf("%u", job->ngsj_attributes->ngsja_authNumber);
    if (string == NULL) {
        ngisErrorPrint(log, fName, "Can't duplicate the string.\n");
        goto finalize;
    }

    string2 = ngisShellQuote(string);
    if (string == NULL) {
        ngisErrorPrint(log, fName, "Can't quote the string.\n");
        goto finalize;
    }
    ...
    fileTransfer = ngisSSHjobFileTransferCreate(
        job, string2,
        job->ngsj_remoteAuthNumberPath,
        NGIS_SSH_JOB_FILE_CREATE_FILE_IN_REMOTE);
...
finalize:
    ...
    NGIS_NULL_CHECK_AND_FREE(string2);
