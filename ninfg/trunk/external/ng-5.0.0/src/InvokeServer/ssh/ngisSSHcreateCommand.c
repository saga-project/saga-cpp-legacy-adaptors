/*
 * $RCSfile: ngisSSHcreateCommand.c,v $ $Revision: 1.3 $ $Date: 2008/03/03 09:11:21 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngisSSHcreateCommand.c,v $ $Revision: 1.3 $ $Date: 2008/03/03 09:11:21 $")
    ...
    ngisSSHjob_t *job)
{
    ...
    if (attr->ngsja_authNumber > 0) {
        result = ngisStringBufferFormat(&sBuf, " --authNumberFile=%s", job->ngsj_remoteAuthNumberPath);
        if (result == 0) {
            ngisErrorPrint(log, fName,
                "Can't append string to the string buffer.\n");
            goto finalize;
