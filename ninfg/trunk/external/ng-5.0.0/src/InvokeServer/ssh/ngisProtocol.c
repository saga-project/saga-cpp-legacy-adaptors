/*
 * $RCSfile: ngisProtocol.c,v $ $Revision: 1.8 $ $Date: 2008/03/03 09:11:21 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngisProtocol.c,v $ $Revision: 1.8 $ $Date: 2008/03/03 09:11:21 $")
...
#define NGISL_REQUEST_ID_QUERY_FEATURES 3
#define NGISL_REQUEST_ID_EXIT           4
...
static ngislRequestInformation_t ngislRequestInformation[] = {
    {NGIS_JOB_CREATE_REQUEST , 1}, 
    {NGIS_JOB_STATUS_REQUEST , 1},
    {NGIS_JOB_DESTROY_REQUEST, 1},
    {NGIS_JOB_QUERY_FEATURES,  0},
    {NGIS_EXIT_REQUEST       , 0}
};

static char *ngislFeatures[] = {
    NGIS_AUTH_NUMBER_FEATURE
};
...
static int ngisProtocolProcessQueryFeatures(ngisProtocol_t *);
    ...
    ngisCallbackResult_t cResult)
{
    ...
    case NGISL_REQUEST_ID_JOB_DESTROY:
        ...
        break;
    case NGISL_REQUEST_ID_QUERY_FEATURES:
	result = ngisProtocolProcessQueryFeatures(protocol);
	break;
...
/**
 * Protocol: Process QUERY_FEATURES Request.
 */
static int
ngisProtocolProcessQueryFeatures(
    ngisProtocol_t *protocol)
{
    int fatal = 0;
    int result;
    ngisLog_t *log;
    static const char fName[] = "ngisProtocolProcessQueryFeatures";

    NGIS_ASSERT(protocol != NULL);

    log = protocol->ngp_log;

    result = ngisProtocolSendQueryFeaturesSuccessReply(
        protocol, ngislProtocolReplyCallback);
    if (result == 0) {
        ngisErrorPrint(log, fName, "Can't send reply.\n");
        fatal = 1;
...
/**
 * Send QUERY_FEATURES Success Reply
 */
int
ngisProtocolSendQueryFeaturesSuccessReply(
    ngisProtocol_t *protocol,
    ngisWriteStringCallbackFunc_t func)
{
    ngisCallback_t callback;
    ...
    int result;
    int i;
    static const char fName[] = "ngisProtocolSendQueryFeaturesSuccessReply";
    ...
    result = ngisStringBufferFormat(&sBuf, "SM%s",
	NGISL_PROTOCOL_LINE_SEPARATOR);
    if (result == 0) {
	ngisErrorPrint(log, fName,
    ...
    result = ngisStringBufferFormat(&sBuf, "protocol_version 2.0%s",
	NGISL_PROTOCOL_LINE_SEPARATOR);
    if (result == 0) {
	ngisErrorPrint(log, fName,
	    "Can't append string to the string buffer.\n");
    ...
    for (i = 0; i < NGIS_NELEMENTS(ngislRequestInformation); i++) {
	result = ngisStringBufferFormat(&sBuf, "request %s%s",
	    ngislRequestInformation[i].ngri_string,
	    NGISL_PROTOCOL_LINE_SEPARATOR);
	if (result == 0) {
	    ngisErrorPrint(log, fName,
		"Can't append string to the string buffer.\n");
    ...
    for (i = 0; i < NGIS_NELEMENTS(ngislFeatures); i++) {
	result = ngisStringBufferFormat(&sBuf, "feature %s%s",
            ngislFeatures[i], NGISL_PROTOCOL_LINE_SEPARATOR);
	if (result == 0) {
	    ngisErrorPrint(log, fName,
		"Can't append string to the string buffer.\n");
	    goto error;
    ...
    result = ngisStringBufferFormat(&sBuf, "REPLY_END%s",
	NGISL_PROTOCOL_LINE_SEPARATOR);
    if (result == 0) {
	ngisErrorPrint(log, fName,
	    "Can't append string to the string buffer.\n");
    ...
    string = ngisStringBufferRelease(&sBuf);
    if (string == NULL) {
	ngisErrorPrint(log, fName,
	    "Can't get the string from string buffer.\n");
	goto error;
    }
...
error:
    callback = ngisCallbackWriteFormat(
	protocol->ngp_stdio.ngsio_out, func, protocol, "F%s",
	NGISL_PROTOCOL_LINE_SEPARATOR);
    if (!ngisCallbackIsValid(callback)) {
        ngisErrorPrint(NULL, fName,
            "Can't register function for sending reply.\n");
