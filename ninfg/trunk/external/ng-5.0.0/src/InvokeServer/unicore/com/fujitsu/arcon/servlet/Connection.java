...
/**
 * Manages connection to a Gateway at the UPL level. UPL messages can be passed
 * to Vsites, the lifetime of the connection can be managed.
 * 
...
public abstract class Connection implements com.fujitsu.arcon.common.Connection {
	...
	/**
	 * Set all new connections following this call to use compression on the ZIP
	 * stream (underlying the UPL connection) when sending data to the Vsite.
	 *  
	 */
	...
	/**
	 * Set all new connections following this call to not use compression on the
	 * ZIP stream (underlying the UPL connection) when sending data to the
	 * Vsite.
	 *  
	 */
	...
	public static Connection getConnection(VsiteTh vsite)
	 ...
	 * 
	 * Will return an existing connection if it exists otherwise will establish
	 * a new connection.
	 * 
	 * @param reference
	 *            The Vsite details.
	 *  
	 */
	public static Connection getConnection(Reference reference)
			throws Connection.Exception {

		// Get the list of (possibly) open connections in this reference
		List l = (List) established_connections.get(reference);

		if (l == null) {
			// Never seen before, initialise the Reference
			l = Collections.synchronizedList(new LinkedList());
			established_connections.put(reference, l);
		}

		// Get any old connection (the first on the list)
		Connection c = null;
		try {
			do {
				c = (Connection) l.remove(0);
			} while (c.closed); // Continue until we get an open one - or all
								// gone
	...
	/**
	 * Should a UPL connection be kept open after use (a UPL trasnaction) or
	 * should it be closed.
	 * 
	 * This is global and will apply to all Connections created after this call.
	 *  
	 */
	...
	public Reply send(Request request) throws Connection.Exception {
		try {
			...
			if (keepOpen()) {
				...
				connect();
				return doSend(request); // If something timed out in Gateway/NJS
										// try again
			} else {
				throw cex;
			}
	...
	private Reply doSend(Request request) throws Connection.Exception {
		...
		try {
			...
			if (keepOpen())
				output.reset(); // Want resends of all instances for new
								// Requests
			...
			_close();
			throw new Connection.Exception("Error sending Request: "
					+ ex.getMessage());
		}
	 ...
	 *  
	 */
	public Reply send(ConsignJob request, PortfolioTh[] portfolios)
			throws Connection.Exception {
		try {
			...
			if (keepOpen()) {
				CLogger.status("Trying again ..." + cex.getMessage());
				connect();
				return doSend(request, portfolios); // If something timed out in
													// Gateway/NJS try again
			} else {
				throw cex;
			}
		}
	...
	private Reply doSend(ConsignJob request, PortfolioTh[] portfolios)
			throws Connection.Exception {
		...
		try {
			...
			//CLogger.status("Sending data with streamed.");

			if (output == null) {
				output = new ObjectOutputStream(socket.getOutputStream());
			}

			output.reset(); // Want resends of all instances for new Requests
			output.writeObject(request);
			output.flush();
			...
			for (int i = 0; i < portfolios.length; i++) {
				...
				String dir_name = portfolios[i].getPortfolio()
						.getUPLDirectoryName()
						+ "/";
				...
				for (int j = 0; j < portfolios[i].getFiles().length; j++) {
					...
					if (portfolios[i].getFiles()[j].exists()) {
						...
						if (portfolios[i].getDestinationNames() == null) {
							// Use name on local system as remote name
						...
						transferFiles(dir_name, portfolios[i].getFiles()[j],
								name, zos);

					} else {
						// This must be a Portfolio marker for no overwrite (add
						// "/" for ZipEntry)
						CLogger.status("Adding no overwrite marker to stream <"
								+ dir_name + "> "
								+ portfolios[i].getFiles()[j].getName());
			...
			throw new Connection.Exception(
					"Error sending Request with streamed: " + ex.getMessage());
		}
	}

	private void transferFiles(String stream_path, File tr_file, String name,
			java.util.zip.ZipOutputStream zos) throws IOException {

		if (tr_file.isDirectory()) {
			File[] files = tr_file.listFiles();
			for (int i = 0; i < files.length; i++) {
				transferFiles(stream_path + name + "/", files[i], files[i]
						.getName(), zos);
			}
		} else {

			// Construct information about file
			...
			int total = 0;
			while (read >= 0) {
				//CLogger.status("Streaming <"+read+"> bytes");
				total += read;
			...
			fis.close();
			zos.closeEntry();
			CLogger.status("Streamed <" + total + "> bytes of: " + file
					+ "in <" + write_time / 1000.0 + "><" + read_time / 1000.0
					+ ">");
		}
	}
	 ...
	 * 
	 * Clears dross from end of a Zip stream (empties packets). Always call at
	 * the end of reading some streamed data from a server.
	 *  
	 */
	public void doneWithStreamedInputStream() {
		if (pis != null) {
			...
			CLogger.status("Skipped <" + skipped
					+ "> bytes at end of PacketisedInputStream");
		}
	}

	/**
	 * Finished with a Connection. This method must be called on all Connections
	 * when they are done with. If this is not called, then this Connection
	 * willl never be available to be returned in other calls of
	 * {@link #getConnection}.
	 *  
	 */
	public void done() {
		...
		if (keepOpen()) {
			// Preserve the reference to this as it is being kept open, could
			// resuse
			if (!gw_list.contains(this)) {
				gw_list.add(this);
				CLogger.status("Keeping connection to "
						+ socket.getInetAddress().getHostName() + " open.");
			}
		} else {
			_close();
		}
	...
	private void _close() {
		...
		CLogger.status("Closing connection to "
				+ socket.getInetAddress().getHostName());
	}

	protected void finalize() {
		// Got here because requesting code forgot about it, treat as done()
		// so that we can still reuse
		done();
	}
	...
	// com.fujitsu.arcon.common.Connection promises
	...
	public Reply sendServerRequest(ServerRequest sr) {
		try {
			...
			CLogger.status("Problem sending request through common interface: "
					+ e.getMessage());
			return null;
		}
	}
	...
	public void writeServerRequest(ServerRequest request) {
		try {
			...
			CLogger.status("Problem writing request through common interface: "
					+ ex.getMessage());
		}
	}
	...
	public Reply readReply() {
		try {
			...
			CLogger.status("Problem reading reply through common interface: "
					+ ex.getMessage());
		}
		return null;
	}
	...
	public void doneWithDataOutputStream() throws IOException {
		try {
			...
			// Ignore this, probably because the ZOS had no entries
			// (coz there was no incarnated Outcome to stream)
		}
	...
	/**
	 * The common interface has done with the Connection, but we may need to
	 * hold it open for other connection attempts, unless there was an error -
	 * when we want to close it down hard.
	 */
