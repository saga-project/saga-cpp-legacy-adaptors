/*
 * $RCSfile: ngProtocolBinary.c,v $ $Revision: 1.17 $ $Date: 2008/03/28 09:36:20 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngProtocolBinary.c,v $ $Revision: 1.17 $ $Date: 2008/03/28 09:36:20 $")
...
static int nglProtocolBinary_MakeArgumentData(
    ...
    ngLog_t *, int *);
static ngCompressionInformationComplex_t* nglCompressionInformationGetComplex(
    ...
    int *error)
{
    ...
    if (sendStream == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't construct Stream Manager.\n"); 
        return 0;
    }
    ...
    item[index++] = protocol->ngp_attr.ngpa_simpleAuthNumber;
    item[index++] = protocol->ngp_connectCount;
	...
	ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "The storage of Negotiation Item overflowed.\n"); 
	return 0;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't write the Negotiation Items to Stream Manager.\n"); 
	return 0;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't write the Conversion Methods to Stream Manager.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't send the Negotiation Result.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't read the Negotiation Information.\n"); 
	return 0;
    }
    ...
    if (nego == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't allocate the storage for Negotiation Information.\n"); 
	return 0;
    }
    ...
    nego->ngpnfe_simpleAuthNumber = item[index++];
    nego->ngpnfe_connectCount     = item[index++];
    nego->ngpnfe_contextID        = item[index++];
    nego->ngpnfe_jobID            = item[index++];
    nego->ngpnfe_executableID     = item[index++];
    ...
    if (index > NGI_NELEMENTS(item)) {
    	NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
	ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "The storage of Negotiation Item underflowed.\n"); 
	goto error;
    }
    ...
    if (nConversions > 0) {
        ...
        if (result == 0) {
    	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	        "Can't read the Conversion Method.\n"); 
	    goto error;
        }
    }
...
error:
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't deallocate the storage for Negotiation Information.\n"); 
	return NULL;
    }
    ...
    int *error)
{
    ...
    nego->ngpnfc_simpleAuthNumber = item[index++];
    nego->ngpnfc_connectCount     = item[index++];
    nego->ngpnfc_contextID        = item[index++];
    nego->ngpnfc_jobID            = item[index++];
    ...
    int *error)
{
    ...
    if (index > NGI_NELEMENTS(item)) {
    	NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
	ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "The storage of Header Item overflowed.\n"); 
	return 0;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't write the Header Items to Stream Manager.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't make the Protocol Header.\n"); 
	return 0;
    }
    ...
    if (paramStream != NULL) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't append the Stream Manager.\n"); 
            return 0;
        }
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't lock the mutex for protocol send\n"); 
        return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't unlock the mutex for protocol send\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    /* Write Keep Connection */
    switch (when) {
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_ARGUMENT_DATA:
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_CALLBACK_RESULT_DATA:
        doClose = 0;
        if (protocol->ngp_attr.ngpa_keepConnect == 0) {
            doClose = 1;
        }
        ...
        result = ngiProtocolBinary_WriteXDRdata(
            protocol, paramStream, NG_ARGUMENT_DATA_TYPE_LONG, 
            &doClose, 1, NULL, NULL, log, error);
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't write the Connection Close.\n"); 
            goto finalize;
        }
    ...
    default:
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Unknown type %d for Transfer Argument Data.\n", when); 
        goto finalize;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't get number of arguments sent.\n"); 
        goto finalize;
    }
    nArguments = nArg;
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write the number of arguments.\n"); 
        goto finalize;
    }
    ...
    do {
        ...
        if (sentHeader == 0) {
            ...
            if (i == arg->nga_nArguments) {
                ...
                if (result == 0) {
                    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                        "Can't get total bytes of readable data "
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't transfer callback's result data.\n"); 
        /* Failed */
        return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write to the Stream Manager.\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write the function information to Stream Manager.\n"); 
	return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "%s:Can't lock the mutex for protocol send\n"); 
        return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "%s:Can't unlock the mutex for protocol send\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    compComplex= nglCompressionInformationGetComplex(
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't encode the Argument Data.\n"); 
	return 0;
    }
    ...
    if (prevTypeOfDivision == NGI_TYPE_OF_DIVISION_NONE) {
        ...
        if (argElement->ngae_sMngRemain == NULL) {
            ...
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't get total bytes of readable data from the StreamManager.\n"); 
                goto error;
            }
            if (isTooLarge != 0) {
                NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Argument's size is too large.\n"); 
            }
        ...
        if ((rmInfo->ngrmi_shrink == 1) &&
            ...
            if (smSkipInfo == NULL) {
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    "Can't construct the Stream Manager.\n"); 
                goto error;
            }
            ...
            if (result ==0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't send the Argument Data of Skip.\n"); 
                return 0;
            }
...
/**
 * Get Compression Information Element
 */
static ngCompressionInformationComplex_t *
nglCompressionInformationGetComplex(
    ngCompressionInformation_t *compInfo,
    int when, 
    int argumentNumber)
{
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write the Skip Information to Stream Manager.\n"); 
        return 0;
    }

    /* Allocate the data buffer */
    skipInfo = ngiMalloc(
        argElement->ngae_nDimensions * sizeof(long) * 4, log, error);
    if (skipInfo == NULL) {
        NGI_SET_ERROR(error, NG_ERROR_MEMORY);
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't allocate the storage for skip Information.\n"); 
        return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write the Shrink Information to Stream Manager.\n"); 

        if (skipInfo != NULL) {
            ngiFree(skipInfo, log, error);
            skipInfo = NULL;
        }
        return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't receive the Protocol Header.\n"); 
        return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Protocol Sequence No. is not valid.\n"); 
        return 0;
    }
	...
	if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "Can't receive the Protocol Parameter.\n"); 
	    return 0;
	}
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't destroy the data it was readed.\n"); 
        return 0;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't read the Protocol Header.\n"); 
        return 0;
    }
    ...
    if (index > NGI_NELEMENTS(item)) {
        NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
        ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "The storage of Header Item underflowed.\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    if ((header->ngph_requestCode >> NGI_PROTOCOL_REQUEST_CODE_NUMSHIFT) !=
        ...
        NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Request type %d is not REQUEST %d.\n",
            header->ngph_requestCode >> NGI_PROTOCOL_REQUEST_CODE_NUMSHIFT, NGI_PROTOCOL_REQUEST_TYPE_REQUEST); 
        return 0;
    }
#endif
    ...
    if (header->ngph_contextID != protocol->ngp_contextID) {
        NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Context ID %ld is not valid.\n", header->ngph_contextID); 
        return 0;
    }
    ...
    if (header->ngph_executableID != protocol->ngp_executableID) {
        NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Executable ID %ld is not valid.\n", header->ngph_executableID); 
        return 0;
    }
    ...
    if (protocol->ngp_sessionID == NGI_SESSION_ID_UNDEFINED) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't set session ID to protocol.\n"); 
            return 0;
        }
    }
    ...
    if (header->ngph_notUsed1 != NGI_PROTOCOL_NOT_USED) {
        NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Not Used1 %ld is not valid.\n", header->ngph_notUsed1); 
        return 0;
    }
    ...
    int *error)
{
    ...
    case NGI_PROTOCOL_REQUEST_CODE_INVOKE_SESSION:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Request of Invoke Session.\n"); 
            return 0;
        }
        break;
    ...
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_ARGUMENT_DATA:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Request of Transfer Argument Data.\n"); 
            return 0;
        }
        break;
    ...
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_CALLBACK_ARGUMENT_DATA:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Request of Transfer Argument Data for Callback.\n"); 
            return 0;
        }
        break;
    ...
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_CALLBACK_RESULT_DATA:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Request of Transfer Result Data for Callback.\n"); 
            return 0;
        }
        break;
    ...
    int *error)
{
    ...
    if (header->ngph_result < 0) {
        ngLogWarn(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Remote executable failed process requested by client.\n"); 
    }
    ...
    case NGI_PROTOCOL_REQUEST_CODE_QUERY_FUNCTION_INFORMATION:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Reply of Query Function Information.\n"); 
            return 0;
        }
        break;
...
# if 0
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Reply of Query Executable Information.\n"); 
            return 0;
        }
        break;
#endif
    ...
    case NGI_PROTOCOL_REQUEST_CODE_PULL_BACK_SESSION:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Reply of Pull Back Session.\n"); 
            return 0;
        }
        break;
    ...
    case NGI_PROTOCOL_REQUEST_CODE_TRANSFER_RESULT_DATA:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Reply of Transfer Result Data.\n"); 
            return 0;
        }
        break;
    ...
    int *error)
{
    ...
    case NGI_PROTOCOL_NOTIFY_CODE_INVOKE_CALLBACK:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't receive the Notify of Invoke Callback.\n"); 
            return 0;
        }
        break;
    ...
    int *error)
{
   ...
   /* Get the Version Number of partner's Protocol */
	...
	if (arg->nga_argument[i].ngae_nDimensions == 0) {
        ...
        arg->nga_argument[i].ngae_subscript = NGI_ALLOCATE_ARRAY(
            ngiSubscriptValue_t, arg->nga_argument[i].ngae_nDimensions,
            log, error);
        if (arg->nga_argument[i].ngae_subscript == NULL) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't allocate the Subscript Value of Argument.\n"); 
            return 0;
        }
    }

    /* Read the Connection Close */
    result = nglProtocolBinary_ReadXDRdataWithReceive(
        protocol, protocol->ngp_sReceive, NG_ARGUMENT_DATA_TYPE_LONG,
	&doClose, 1, &readNbytesFromStream, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't read the Number Of Arguments.\n"); 
        return 0;
    }

    /* Set the Connection Close */
    result = ngiProtocolSetConnectionClose(
        protocol, doClose, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't set the Connection Close.\n"); 
        return 0;
    }
    ...
    if (nArgs < 0) {
        ...
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Argument count %ld is not valid.\n", nArgs); 
        return 0;
    }
    ...
    if (nArgs > arg->nga_nArguments) {
        NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Argument count %ld over the Method Info Argument Information.\n", nArgs); 
        return 0;
    }
    ...
    for (i = 0; i < arg->nga_nArguments; i++) {
        ...
        if (index > NGI_NELEMENTS(item)) {
            NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
            ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "The storage of Argument Data Item overflowed.\n"); 
            return 0;
        }
        ...
        if (argHead.ngpad_no != (i + 1)) {
            NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Argument No. is not valid: Require %d, receive %ld.\n",
                i + 1, argHead.ngpad_no); 
            return 0;
        }
        ...
        if (argHead.ngpad_type != arg->nga_argument[i].ngae_dataType) {
            NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Data type is not valid: Require %d, receive %ld.\n",
                arg->nga_argument[i].ngae_dataType, argHead.ngpad_type); 
            return 0;
        }
        ...
        if (argHead.ngpad_ioMode != arg->nga_argument[i].ngae_ioMode) {
            NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "I/O mode is not valid: Require %d, receive %ld.\n",
                arg->nga_argument[i].ngae_ioMode, argHead.ngpad_ioMode); 
            return 0;
        }
        ...
        if ((argHead.ngpad_encode == NGI_CONVERSION_SKIP) ||
            ...
            if (result ==0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't receive the Argument Data of Skip.\n"); 
                return 0;
            }
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't decode the Argument Data.\n"); 
            return 0;
        }
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't initialize the Argument Data.\n"); 
            return 0;
        }
    }
    ...
    for (i = 0; i < arg->nga_nArguments; i++) {
        if (arg->nga_argument[i].ngae_subscript != NULL) {
            result = NGI_DEALLOCATE(ngiSubscriptValue_t,
                arg->nga_argument[i].ngae_subscript, log, error);
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    "Can't construct the Subscript Value.\n"); 
                return 0;
            }
    ...
    int *error)
{
    ...
    if (callbackID != callbackIDofProtocol) {
        NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Received callback ID is not valid.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    ngiProtocolArgumentData_t argHead;
    long nArgs, item[NGI_PROTOCOL_ARGUMENT_DATA_NITEMS];
    long callbackID, callbackIDofProtocol, doClose;
    int index;
    int count = 0;
    int i;
    int result;
    ...
    if ((i == rmInfo->ngrmi_nArguments) && (count != callbackIDofProtocol)) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Callback ID %ld is not valid.\n", callbackIDofProtocol); 
        return 0;
    }
    ...
    if (arg == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't get the Argument from protocol.\n"); 
        return 0;
    }
	...
	/* Receive the items */
        ...
        if (argHead.ngpad_type !=
            ...
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Data type is not valid: Require %d, receive %ld.\n",
                rmInfoCallback->ngrmi_arguments[i].ngai_dataType, argHead.ngpad_type); 
            return 0;
        }
        ...
        if (argHead.ngpad_ioMode !=
            ...
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "I/O mode is not valid: Require %d, receive %ld.\n",
                rmInfoCallback->ngrmi_arguments[i].ngai_ioMode, argHead.ngpad_ioMode); 
            return 0;
        }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't receive the Session Information.\n"); 
        return 0;
    }
    ...
    if (protocol->ngp_rcInfo != NULL) {
        ngLogWarn(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Remote Class Information is already set.\n"); 
    }
    ...
    int *error)
{
    ...
    buf = ngiCalloc(1, head->ngph_nBytes, log, error);
    if (buf == NULL) {
    	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't allocate the storage for Receive Data.\n"); 
	return 0;
    }
    ...
    *information = calloc(
    ...
    if (*information == NULL) {
    	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't allocate the storage for Function Information.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    ngLogInfo(log, NG_LOGCAT_NINFG_PURE, fName,  
        "Session Information as below.\n\"%s\"\n", buf); 

    /* Convert the Session Information */
    result = ngiProtocolGetSessionInformationFromXML(
        protocol, buf,
   ...
   /* Get the Version Number of partner's Protocol */
	...
	/* Receive the items */
        ...
        if (argHead.ngpad_type != rmInfo->ngrmi_arguments[i].ngai_dataType) {
            ...
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Data type is not valid: Require %d, receive %ld.\n",
                rmInfo->ngrmi_arguments[i].ngai_dataType, argHead.ngpad_type); 
            return 0;
        }
        ...
        if (argHead.ngpad_ioMode != rmInfo->ngrmi_arguments[i].ngai_ioMode) {
            ...
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "I/O mode is not valid: Require %d, receive %ld.\n",
                rmInfo->ngrmi_arguments[i].ngai_ioMode, argHead.ngpad_ioMode); 
            return 0;
        }
    ...
    int *error)
{
	...
	if (arg->nga_argument[i].ngae_nDimensions == 0) {
        ...
        arg->nga_argument[i].ngae_subscript = NGI_ALLOCATE_ARRAY(ngiSubscriptValue_t,
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't set the Argument Data of Callback.\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't read the Skip Information.\n"); 
        return 0;
    }

    /* Allocate the data buffer */
    skipInfo = ngiMalloc(nDim * sizeof(long) * 4, log, error);
    if (skipInfo == NULL) {
        NGI_SET_ERROR(error, NG_ERROR_MEMORY);
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't allocate the storage for skip Information.\n"); 
    ...
    for (i = 0; i < argElement->ngae_nDimensions; i++) {
        if ((argElement->ngae_subscript[i].ngsv_start >
            ...
            NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "The value of start of subscript is larger than size.(start = %d, size = %d)\n",
                argElement->ngae_subscript[i].ngsv_start, argElement->ngae_subscript[i].ngsv_size); 
            return 0;
        } else if (argElement->ngae_subscript[i].ngsv_start >
            argElement->ngae_subscript[i].ngsv_end) {
            NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "The value of start of subscript is larger than end.(start = %d, end = %d)\n",
                argElement->ngae_subscript[i].ngsv_start, argElement->ngae_subscript[i].ngsv_end); 
            return 0;
        } else if (argElement->ngae_subscript[i].ngsv_end >
            argElement->ngae_subscript[i].ngsv_size) {
            NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "The value of end of subscript is larger than size.(end = %d, size = %d)\n",
                argElement->ngae_subscript[i].ngsv_end, argElement->ngae_subscript[i].ngsv_size); 
            return 0;
        }
    }
    ...
    int *error)
{
    ...
    if ((result == 0) || ngiSizetInvalidCheck(nBytes < 0)) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't get the sizeof data.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if ((result == 0) || ngiSizetInvalidCheck(sizeofNativeData < 0)) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't get the sizeof data of native.\n"); 
	goto error;
    }
    ...
    if ((result == 0) || ngiSizetInvalidCheck(sizeofNetData < 0)) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't get the sizeof data.\n"); 
	goto error;
    }
	...
	*networkNbytes = sizeofNetData * nElements;
	...
	if (result == 0) {
	    ngLogFatal(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "Can't get the Writable Buffer from Stream Manager.\n"); 
	    goto error;
	}
    ...
    int *error)
{
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't encode the string to XDR.\n"); 
	goto error;
    }
    *networkNbytes = xdrNbytes;
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't write the data to Stream Manager.\n"); 
        goto error;
    }
    ...
    int *error)
{
	...
	*readNbytesFromStream = 0;
    ...
    if ((result == 0) || ngiSizetInvalidCheck(sizeofNetData < 0)) {
    	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    	    "Can't get the sizeof data of network.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    *readNbytesFromStream = nBytes;
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't read from the Stream Manager.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't receive the data from socket.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    *readNbytesFromStream = 0;
	...
	if (canReadNbytes >= sizeofNetData) {
	    ...
	    if (result == 0) {
		ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
		    "Can't read the data from Stream Buffer.\n"); 
		goto error;
	    }
	    *readNbytesFromStream += canReadNbytes;
	    ...
	    if (result == 0) {
		ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
		    "Can't read the data from Stream Manager.\n"); 
		goto error;
	    }
    ...
    int *error)
{
    ...
    *readNbytesFromSocket = 0;
	...
	/* Read the length of string */
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't read the length of string.\n"); 
            goto error;
        }
        nBytes = stringLength;
    ...
    *string = ngiMalloc(nBytes + 1, log, error);
    if (*string == NULL) {
	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	...
	if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "Can't receive the string from socket.\n"); 
	    goto error;
	}
    }
    ...
    int *error)
{
   ...
   /* Initialize the NET Communicator */
    ...
    xdrNbytes = nBytes;
    xdrBuff = ngiMalloc(xdrNbytes + 1, log, error);
    if (xdrBuff == NULL) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	...
	if (xdrNbytes != receiveNbytes) {
	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "Data length is not correct: require = %lu, receive = %lu).\n",
                (unsigned long)xdrNbytes, (unsigned long)receiveNbytes); 
	    goto error;
	}
	*readNbytesFromStream += receiveNbytes;
	...
	if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "Can't decode the string from XDR to native.\n"); 
	    goto error;
	}
    ...
    /* Copy the string */
    *string = ngiMalloc(*strNbytes + 1, log, error);
    if (*string == NULL) {
	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    ...
    int *error)
{
   ...
   /* Initialize the NET Communicator */
    ...
    /* Allocate the storage for XDR */
    xdrNbytes = nBytes;
    xdrBuff = ngiMalloc(xdrNbytes, log, error);
    if (xdrBuff == NULL) {
	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't allocate the storage for XDR.\n"); 
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't read the XDR data from Stream Manager.\n"); 
	goto error;
    }
