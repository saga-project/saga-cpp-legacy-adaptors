/*
 * $RCSfile: ngArgumentConvert.c,v $ $Revision: 1.14 $ $Date: 2008/03/28 09:26:27 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngArgumentConvert.c,v $ $Revision: 1.14 $ $Date: 2008/03/28 09:26:27 $")
    ...
    int *error)
{
	...
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't allocate the storage for Argument.\n"); 
	return NULL;
    }
...
error:
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't deallocate the storage for Argument.\n"); 
	return NULL;
    }
...
nglArgumentAllocate(int nArguments, ngLog_t *log, int *error)
{
	...
	(sizeof (ngiArgumentElement_t) * nArguments), log, error);
    if (arg == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't allocate the storage for Argument.\n"); 
	return NULL;
    ...
    int *error)
{
	...
	*value = referArg->nga_argument[position].ngae_data.ngad_long;
	...
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Data type %d is not supported.\n",
            referArg->nga_argument[position].ngae_dataType); 
	return 0;
    }
    ...
    int *error)
{
	...
	if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "Can't destruct the Subscript Value.\n"); 
	    return 0;
	}
        ...
        for (j = 0; j < arg->nga_argument[i].ngae_nDimensions; j++) {
            ...
            if ((arg->nga_argument[i].ngae_subscript[j].ngsv_start >
                ...
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    ...
                    arg->nga_argument[i].ngae_subscript[j].ngsv_start,
                    arg->nga_argument[i].ngae_subscript[j].ngsv_size); 
                return 0;
    ...
    int *error)
{
    ...
    if (argElement == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Argument Element.\n"); 
	return NULL;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't finalize the Argument Element.\n"); 
	return 0;
    }
...
nglArgumentElementAllocate(int nArguments, ngLog_t *log, int *error)
{
    ...
    /* Allocate */
    argElement = ngiCalloc(
	nArguments, sizeof (ngiArgumentElement_t), log, error);
    if (argElement == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Argument Element.\n"); 
	return NULL;
    ...
    int *error)
{
	...
	if (arg->nga_argument[i].ngae_dataType == NG_ARGUMENT_DATA_TYPE_STRING) {
		...
		NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
		ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
		    "Unknown data type %d.\n", arg->nga_argument[i].ngae_dataType); 
		return 0;
	    }
    ...
    int *error)
{
	...
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Too few arguments (%d < %d).\n", argStack->ngas_nargs, nArgs); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't get the argument data and pointer.\n"); 
	return 0;
    }
	...
	case NG_ARGUMENT_DATA_TYPE_CALLBACK:
	    NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	    ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "This data type %d could not use array.\n", argElement->ngae_dataType); 
	    return 0;
    ...
    int *error)
{
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't deallocate the storage for data.\n"); 
	return 0;
    }
    ...
    if (argElement->ngae_tmpFileNameTable != NULL) {
        ngiFree(argElement->ngae_tmpFileNameTable, log, error);
...
copy:
    ...
    case NG_ARGUMENT_DATA_TYPE_DCOMPLEX:
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get the size of data.\n"); 
            return 0;
        }

        /* Allocate the memory */
	argp.ngap_void = ngiCalloc(
            argElement->ngae_nElements, nBytes, log, error);
    ...
    case NG_ARGUMENT_DATA_TYPE_STRING:
		...
		goto success;
            ...
            /* Allocate the memory */
            argp.ngap_stringArray = ngiCalloc(
                argElement->ngae_nElements, sizeof (char *), log, error);
		    ...
		    continue;
                ...
                /* Allocate the memory */
                argp.ngap_stringArray[i] = ngiCalloc(1,
    ...
    case NG_ARGUMENT_DATA_TYPE_FILENAME:
		...
		goto success;
            ...
            /* Allocate the memory */
            argp.ngap_fileNameArray = ngiCalloc(
                argElement->ngae_nElements, sizeof (char *), log, error);
		    ...
		    continue;
                ...
                /* Allocate the memory */
                argp.ngap_fileNameArray[i] = ngiCalloc(1,
...
cantAllocate:
    ...
    if ((argElement->ngae_dataType == NG_ARGUMENT_DATA_TYPE_STRING) &&
		...
		ngiFree(argp.ngap_stringArray[i], log, NULL);
		argp.ngap_stringArray[i] = NULL;
	    }
        }
    }
    ...
    if ((argElement->ngae_dataType == NG_ARGUMENT_DATA_TYPE_FILENAME) &&
		...
		ngiFree(argp.ngap_fileNameArray[i], log, NULL);
		argp.ngap_fileNameArray[i] = NULL;
	    }
        }
    }
    ...
    /* Deallocate */
    if (argp.ngap_void != NULL) {
        ngiFree(argp.ngap_void, log, NULL);
        argp.ngap_void = NULL;
    }
    ...
    int *error)
{
    ...
    if ((argElement->ngae_dataType == NG_ARGUMENT_DATA_TYPE_STRING) &&
        ...
        for (i = 0; i < argElement->ngae_nElements; i++) {
            if (argElement->ngae_pointer.ngap_stringArray[i] == NULL)
                continue;
            ngiFree(argElement->ngae_pointer.ngap_stringArray[i], log, error);
    ...
    if ((argElement->ngae_dataType == NG_ARGUMENT_DATA_TYPE_FILENAME) &&
        ...
        for (i = 0; i < argElement->ngae_nElements; i++) {
            if (argElement->ngae_pointer.ngap_fileNameArray[i] == NULL)
                continue;
            ngiFree(argElement->ngae_pointer.ngap_fileNameArray[i], log, error);
    ...
    if (argElement->ngae_pointer.ngap_void != NULL) {
        ngiFree(argElement->ngae_pointer.ngap_void, log, error);
        argElement->ngae_pointer.ngap_void = NULL;
    }
    ...
    int *error)
{
    ...
    /* Allocate */
    if (nElements > 0) {
        data = ngiMalloc(nBytes * nElements, log, error);
        if (data == NULL) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't allocate the storage for Argument Data.\n"); 
            return 0;
            ...
            if (argElement->ngae_ioMode == NG_ARGUMENT_IO_MODE_IN) {
                ...
                for (i = 0; i < argElement->ngae_nElements; i++) {
                    if (stringArray[i] != NULL) {
                        ngiFree(stringArray[i], log, error);
                        stringArray[i] = NULL;
                    }
                }
            }
            ...
            if (argElement->ngae_ioMode == NG_ARGUMENT_IO_MODE_IN) {
                ...
                for (i = 0; i < argElement->ngae_nElements; i++) {
                    if (fileNameArray[i] != NULL) {
                        ngiFree(fileNameArray[i], log, error);
    ...
    int *error)
{
    ...
    if (argElement->ngae_typeOfDivision == NGI_TYPE_OF_DIVISION_NONE) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't append header to the argument data.\n"); 
            goto error;
        }
    }
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't divide the argument data.\n"); 
        goto error;
    }
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't compress the argument data.\n"); 
        goto error;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't destruct the Stream Manager read already.\n"); 
        goto error;
    }
    ...
    if (smPartial == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't construct the Partial Stream Manager.\n"); 
        goto error;
    }
    ...
    int *error)
{
	...
	((argElement->ngae_networkTotalNbytesIsVeryLarge != 0) ||
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't encode of Zlib the argument data.\n"); 
            goto error;
        }
    ...
    int *error)
{
    ...
    if ((rmInfo->ngrmi_shrink == 1) && (argElement->ngae_subscript != NULL)) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't encode of skip the argument data.\n"); 
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't encode the argument data of skip.\n"); 
            goto error;
        }

        assert(skipData != NULL);
        ngiFree(skipData, log, error);
        skipData = NULL;
    } else {
        result = ngiProtocolBinary_WriteData(
    ...
    int *error)
{
	...
	if (strNbytes < 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	        "String length (%ld) is less than zero.\n", strNbytes); 
	    return 0;
	}
    }
    ...
    int *error)
{
    ...
    for (i = 0; i < argElement->ngae_nElements; i++) {
        ...
        if (fileNameArray[i] == NULL) {
            ...
            sMngFile = ngiFileStreamManagerConstruct(
                fileNameArray[i], protocol->ngp_event, log, error);
            if (sMngFile == NULL) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't construct the File Stream Manager.\n"); 
                goto error;
            }
            ...
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't get number of readable bytes from the File Stream Manager.\n"); 
                goto error;
            }
    ...
    int *error)
{
    ...
    if (sMngDest == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't construct the Stream Manager for receiving.\n"); 
        goto error;
    }
    ...
    while (loop != 0) {
       ...
       readTotalBytes += readNbytes;
        ...
        default:
            ...
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Receive invalid value of conversion method.\n"); 
            goto error;
        }
        ...
        case NGI_BYTE_STREAM_CONVERSION_DIVIDE:
            ...
            default:
                NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Receive invalid value of type of division.\n"); 
                goto error;
            }
            break;
        ...
        case NGI_BYTE_STREAM_CONVERSION_ZLIB:
            ...
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't decode of Zlib the argument data.\n"); 
                goto error;
            }
            /* Destroy the Stream Manager */
            result = ngiStreamManagerDestruct(
    ...
    int *error)
{
    ...
    if ((argHeader->ngpad_encode == NGI_CONVERSION_SKIP) ||
        ...
        if (argHeader->ngpad_nElements > 0) {
            data = ngiMalloc(
                sizeofNativeData * argHeader->ngpad_nElements, log, error);
            if (data == NULL) {
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    "Can't allocate the storage for Decode of skip.\n"); 
                goto error;
            }
        }
    ...
    int *error)
{
    ...
    if (result == 0) {
        NGI_SET_ERROR(error, NG_ERROR_MEMORY);
        ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
            "Can't allocate the storage for array of string.\n"); 
        return 0;
    }
    ...
    */
    ...
    if (allocate != 0) {
        ...
        if (argHead->ngpad_nElements > 0) {
            fileNameArray = ngiMalloc(
                argHead->ngpad_nElements * sizeof(char *), log, error);
            if (fileNameArray == NULL) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                    "Can't allocate the storage for Argument Data.\n"); 
                goto error;
...
error:
    /* Free fileNameArray? */
    if (fileNameArray != NULL) {
        ngiFree(fileNameArray, log, NULL);
        fileNameArray = NULL;
    }
    ...
    int *error)
{
    ...
    if (nElements != 0) {
        ...
        buf = ngiMalloc(nBytes * nElements, log, error);
        if (buf == NULL) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't allocate the storage for Argument Data.\n"); 
            return 0;
    ...
    int *error)
{
    ...
    ngLogInfo(log, NG_LOGCAT_NINFG_PURE, fName,  
        "Before compress Data = %lu Bytes, After compress Data = %lu Bytes, The rate of compression = %f%%.\n",
        z.total_in, z.total_out, ((double)z.total_out / (double)z.total_in) * 100.0); 
    ...
    int *error)
{
    ...
    if (srcSize < 0) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get number of bytes of total data from the Stream Manager\n"); 
            goto error;
        }
    } else {
	srcNbytes = srcSize;
    ...
    do {
        if (z.avail_in == 0) {
            ...
            if (canReadNbytes > 0) {
                ...
                if (result == 0) {
                    ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                        "Can't read Buffer from Stream Manager.\n"); 
                    goto error;
                }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't convert the argument data.\n"); 
	return 0;
    }
#endif
    ...
    int *error)
{
    ...
    /* Allocate the data buffer */
    buf = ngiMalloc(nBytes, log, error);
    if (buf == NULL) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't allocate the storage for Argument Data.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    /* Allocate */
    subValue = NGI_ALLOCATE_ARRAY(ngiSubscriptValue_t, nSubInfos, log, error);
    if (subValue == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Subscript Value.\n"); 
	return NULL;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't initialize the Subscript Value.\n"); 
	goto error;
    }
...
error:
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiSubscriptValue_t, subValue, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Subscript Value.\n"); 
	return NULL;
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't finalize the Subscript Value.\n"); 
	return 0;
    }

    /* Deallocate */
    result = NGI_DEALLOCATE(ngiSubscriptValue_t, subValue, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Subscript Value.\n"); 
	return 0;
...
#define NGI_STACK_SIZE 10
    ...
    *answer = 0;
	...
	case NG_EXPRESSION_VALUE_TYPE_IN_ARGUMENT:
	    ...
	    if (result == 0) {
	    	ngLogFatal(log, NG_LOGCAT_NINFG_PURE, fName,  
	    	    "Can't get the value of Argument.\n"); 
		abort();
	    	return 0;
	    }
	...
	default:
	    ...
	    NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	    ngLogFatal(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "Operation Code %d is not defined.\n", (int)expression[i].ngee_value); 
	    abort();
	    return 0;
...
ngiStackIntConstruct(int sizeOfStack, ngLog_t *log, int *error)
{
    ...
    if (stack == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stack.\n"); 
	return NULL;
    }
...
error:
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stack.\n"); 
	goto error;
    }
...
ngiStackIntAllocate(int sizeOfStack, ngLog_t *log, int *error)
{
	...
	+ (sizeof (stack->ngsi_data[0]) * sizeOfStack), log, error);
    if (stack == NULL) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't allocate the storage for Stack.\n"); 
	return NULL;
