/*
 * $RCSfile: ngStream.c,v $ $Revision: 1.17 $ $Date: 2008/02/07 10:26:15 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: ngStream.c,v $ $Revision: 1.17 $ $Date: 2008/02/07 10:26:15 $")
...
ngiStreamManagerTypeInformation_t nglMemoryStreamManagerTypeInformation = {
    /* Destroy Data */
...
#define nglStreamManagerDestruct(sMng, log, error) \
    ((sMng)->ngsm_typeInformation->ngsmti_destruct(sMng, log, error))
...
ngiMemoryStreamManagerConstruct(size_t growNbytes, ngLog_t *log, int *error)
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiMemoryStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stream Manager.\n"); 
	return NULL;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't initialize the Stream Manager.\n"); 
	goto error;
    }
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiMemoryStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
    }
...
ngiStreamManagerDestruct(ngiStreamManager_t *sMng, ngLog_t *log, int *error)
{
    ...
    while (curMng != NULL) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't destruct the Stream Manager.\n"); 
            return 0;
        }
        curMng = nextMng;
    }
...
ngiStreamManagerDestructReadAlready(ngiStreamManager_t **sMng, ngLog_t *log, int *error)
{
    ...
    while (curMng != NULL) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get readable buffer from the Stream Manager.\n"); 
            return 0;
        }
        if (readableNbytes > 0) {
            break;
...
nglMemoryStreamManagerDestruct(ngiStreamManager_t *sMng, ngLog_t *log, int *error)
{
    ...
    /* Check the arguments */
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglMemoryStreamManagerTypeInformation);
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't finalize the Stream Manager.\n"); 
	return 0;
    }

    /* Deallocate */
    result = NGI_DEALLOCATE(ngiMemoryStreamManager_t, msMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    ngiStreamManagerInitialize(&msMng->ngmsm_base, &nglMemoryStreamManagerTypeInformation, log, error);
...
nglMemoryStreamManagerGrowWrite(ngiMemoryStreamManager_t *msMng, ngLog_t *log, int *error)
{
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't register the Stream Buffer.\n"); 
	return 0;
    }
...
nglMemoryStreamManagerGrow(ngiMemoryStreamManager_t *msMng, ngLog_t *log, int *error)
{
    ...
    if (stream == NULL) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't construct the Stream Buffer.\n"); 
	return 0;
    }
    ...
    for (*nBytes = 0; sMng != NULL; sMng = sMng->ngsm_next) {
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                "Can't get number of bytes of readable data.\n"); 
            return 0;
        }
        *nBytes += n;
    ...
    int *error)
{
    ...
    while (1) {
        ...
        while ((smCurr != NULL) && (canReadNbytes == 0)) {
            ...
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    "Can't get the Readable Buffer from Stream Manager.\n"); 
                return 0;
            }
            if (canReadNbytes == 0) {
    ...
    int *error)
{
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't get the total bytes of remainig data.\n"); 
	return 0;
    }
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
	    "Can't write to the Stream Buffer.\n"); 
	return 0;
    }
    ...
    int *error)
{
    ...
    if (stream == NULL) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't allocate the storage for Stream Buffer.\n"); 
	return NULL;
    }
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Buffer.\n"); 
	goto error;
    }
...
error:
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't deallocate the storage for Stream Buffer.\n"); 
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogFatal(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the Stream Buffer.\n"); 
	return 0;
    }
    ...
    int *error)
{
	...
	/* Get the Stream Buffer */
        ...
        if (result == 0) {
            ngLogFatal(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get writable buffer from the Stream Manager.\n"); 
            return 0;
        }
    ...
    int *error)
{
	...
	/* Get the Stream Buffer */
        ...
        if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "Can't get the readable Stream Manager.\n"); 
            return 0;
        }
    ...
    int *error)
{
	...
	if (readNbytes + remainNbytes > nBytes) {
        ...
        if (result == 0) {
	    ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "Can't write the Stream Manager.\n"); 
	    return 0;
        }
    ...
    int *error)
{
    ...
    *newStream = NULL;
	...
	if (stream->ngsb_writeNbytes < stream->ngsb_readNbytes) {
	    NGI_SET_ERROR(error, NG_ERROR_OVERFLOW);
	    ngLogFatal(log, NG_LOGCAT_NINFG_PURE, fName,  
	        "Read bytes %d greater than written bytes %d.\n", stream->ngsb_readNbytes, stream->ngsb_writeNbytes); 
	    return 0;
	}
    ...
    int *error)
{
    ...
    result = ngiStreamBufferIsValid(stream, log, error);
    if (result == 0) {
	ngLogError(ngLog, NG_LOGCAT_NINFG_PURE, fName,  
	    "Stream Buffer is not valid.\n"); 
	return -1;
    }
    ...
    int *error)
{
	...
	NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	ngLogError(ngLog, NG_LOGCAT_NINFG_PURE, fName,  
	    "The number of bytes of buffer is smaller equal zero.\n"); 
	return 0;
    }
	...
	NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	ngLogError(ngLog, NG_LOGCAT_NINFG_PURE, fName,  
	    "The buffer is smaller than nBytes written.\n"); 
	return 0;
    }
...
ngiStreamManagerInitialize(ngiStreamManager_t *sMng, ngiStreamManagerTypeInformation_t *typeInfo, ngLog_t *log, int *error)
{
    ...
    sMng->ngsm_typeInformation = typeInfo;
    sMng->ngsm_next = NULL;
...
nglStreamManagerInitializePointer(ngiStreamManager_t *sMng)
{
    ...
    sMng->ngsm_typeInformation = NULL;
    sMng->ngsm_next = NULL;
}
...
ngiStreamManagerTypeInformation_t nglFileStreamManagerTypeInformation = {
    /* Destroy Data */
    ...
    int *error)
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiFileStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for File Stream Manager.\n"); 
	return NULL;
    }

    /* Initialize */
    result = ngiFileStreamManagerInitialize(sMng, fileName, event, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't initialize the Stream Manager.\n"); 
	goto error;
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiFileStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    /* Check the arguments */
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglFileStreamManagerTypeInformation);
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiFileStreamManager_t, fsMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)fsMng,
        &nglFileStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    fsMng->ngfsm_handle = handle;

    fileName = fsMng->ngfsm_fileName;

    ngLogDebug(log, NG_LOGCAT_NINFG_PURE, fName,  
        "Open the file named \"%s\" for read.\n", fileName); 

    /* Open file */
    result = ngiIOhandleFileOpen(
        handle, fileName, NGI_IOHANDLE_FILE_OPEN_TYPE_READ, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
...
error:
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the File Stream Manager.\n"); 
    }
    return 0;
}
    ...
    int *error)
{
    ...
    if (fsMng->ngfsm_handleInitialized != 0) {
        ...
        result = ngiIOhandleDestruct(fsMng->ngfsm_handle, log, error);
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    ...
    int *error)
{
	...
	NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Number of bytes required is too large.\n"); 
	return  0;
    }
    ...
    if ((readableNbytes == 0) &&
        ...
        if (fileReadableNbytes > 0) {
            result = ngiIOhandleRead(
                fsMng->ngfsm_handle, (char *)stream->ngsb_pointer,
                fileReadableNbytes, fileReadableNbytes, &readNbytes,
                log, error);
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
        ...
        readableNbytes = stream->ngsb_writeNbytes = readNbytes;
        stream->ngsb_readNbytes = 0;
    }
...
ngiStreamManagerTypeInformation_t nglDelimiterStreamManagerTypeInformation = {
    /* Destroy Data */
    ...
    int *error)
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiDelimiterStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Delimiter Stream Manager.\n"); 
	return NULL;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't initialize the Delimiter Stream Manager.\n"); 
	goto error;
    }
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiDelimiterStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Delimiter Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    /* Check the arguments */
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglDelimiterStreamManagerTypeInformation);
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiDelimiterStreamManager_t, psMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)dsMng,
        &nglDelimiterStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the Delimiter Stream Manager.\n"); 
        return 0;
    }
    ...
    int *error)
{
	...
	NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Number of bytes read is too large.\n"); 
	return  0;
    }
...
ngiStreamManagerTypeInformation_t nglPartialStreamManagerTypeInformation = {
    /* Destroy Data */
    ...
    int *error)
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiPartialStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Partial Stream Manager.\n"); 
	return NULL;
    }
    ...
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't initialize the Partial Stream Manager.\n"); 
	goto error;
    }
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiPartialStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Partial Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    /* Check the arguments */
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglPartialStreamManagerTypeInformation);
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiPartialStreamManager_t, psMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)psMng,
        &nglPartialStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    /* Delimiter */
    if ((smWhole != NULL) &&
        (smWhole->ngsm_typeInformation
            == &nglDelimiterStreamManagerTypeInformation)) {
        result = ngiStreamManagerGetReadableBuffer(
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't read buffer from the Stream Manager.\n"); 
            return 0;
        }
    }
    ...
    int *error)
{
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the Partial Stream Manager.\n"); 
        return 0;
    }
    ...
    int *error)
{
    ...
    for (smReadable = psMng->ngpsm_smWhole;
         (smReadable != NULL) &&
         (smReadable->ngsm_typeInformation != &nglDelimiterStreamManagerTypeInformation);
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get readable buffer from whole Stream Buffer.\n"); 
            return  0;
        }
    ...
    int *error)
{
    ...
    if ((remainNbytes < nBytes) ||
        (psMng->ngpsm_smWhole == NULL) ||
        (psMng->ngpsm_smWhole->ngsm_typeInformation ==
         &nglDelimiterStreamManagerTypeInformation)) {
	NGI_SET_ERROR(error, NG_ERROR_INVALID_ARGUMENT);
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Number of bytes read is too large.\n"); 
    ...
    if (result == 0) {
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't read buffer from whole Stream Buffer.\n"); 
	return  0;
    }
    ...
    int *error)
{
    ...
    for (smCur = psMng->ngpsm_smWhole;
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get number of bytes of readable.\n"); 
            return 0;
        }

        if (smCur->ngsm_typeInformation
            == &nglDelimiterStreamManagerTypeInformation) {
            break;
        }
        if (tmpIsTooLarge != 0) {
...
ngiStreamManagerTypeInformation_t nglReceivingStreamManagerTypeInformation = {
    /* Destroy Data */
    ...
    int *error) 
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiReceivingStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stream Manager.\n"); 
	return NULL;
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiReceivingStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)rsMng,
        &nglReceivingStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    int *error)
{
    ...
    assert(buf != NULL);
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglReceivingStreamManagerTypeInformation);
    ...
    int *error)
{
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiReceivingStreamManager_t, rsMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
...
ngiStreamManagerTypeInformation_t nglStringReceivingStreamManagerTypeInformation = {
    /* Destroy Data */
    nglNonImplementationStreamManagerDestroyWriteData,
    nglNonImplementationStreamManagerDestroyReadData,
    ...
    int *error) 
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiStringReceivingStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stream Manager.\n"); 
	return NULL;
...
error:
    if (sMng != NULL) {
        result = NGI_DEALLOCATE(ngiStringReceivingStreamManager_t, sMng, log, NULL);
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't deallocate the storage for Stream Manager.\n"); 
        }
    ...
    int *error)
{
    ...
    /* Initialize the base and the members */
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)rsMng,
        &nglStringReceivingStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
...
error:
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the String Receiving Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    assert(buf != NULL);
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglStringReceivingStreamManagerTypeInformation);
        ...
        case NGL_STRING_RECEIVING_STATE_SIZE:
            if (rsMng->ngsrsm_writeNbytes == rsMng->ngsrsm_protocol->ngp_xdrDataSize.ngds_long) {
                ...
                if (rsMng->ngsrsm_length < 0) {
                    ...
                    ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
                        "The string length is less than zero.\n"); 
                    goto error;
                } 
                if (rsMng->ngsrsm_length == 0) {
                    /* Null String */
    ...
    int *error)
{
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiStringReceivingStreamManager_t, rsMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
...
ngiStreamManagerTypeInformation_t nglFileReceivingStreamManagerTypeInformation = {
    /* Destroy Data */
    nglNonImplementationStreamManagerDestroyWriteData,
    nglNonImplementationStreamManagerDestroyReadData,
    ...
    int *error) 
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiFileReceivingStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stream Manager.\n"); 
	return NULL;
...
error:
    if (sMng != NULL) {
        result = NGI_DEALLOCATE(ngiFileReceivingStreamManager_t, sMng, log, NULL);
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't deallocate the storage for Stream Manager.\n"); 
        }
    ...
    int *error)
{
    ...
    /* Initialize the base and the members */
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)rsMng,
        &nglFileReceivingStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    rsMng->ngfrsm_buffer = ngiMalloc(rsMng->ngfrsm_bufferSize, log, error);
    if (rsMng->ngfrsm_buffer == NULL) {
    	NGI_SET_ERROR(error, NG_ERROR_MEMORY);
	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
	    "Can't allocate the storage for receiving files.\n"); 
        goto error;
    }
...
error:
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the File Receiving Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    if (rsMng->ngfrsm_buffer != NULL) {
        ngiFree(rsMng->ngfrsm_buffer, log, error);
        rsMng->ngfrsm_buffer = NULL;
    }
    ...
    int *error)
{
    ...
    assert(buf != NULL);
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglFileReceivingStreamManagerTypeInformation);
    ...
    if (rsMng->ngfrsm_fileHeaderRead == 0) {
        ...
        if (nBytes > 0) {
            result = ngiIOhandleWrite(
                rsMng->ngfrsm_handle, (char *)rsMng->ngfrsm_buffer,
                    nBytes, nBytes, &writeNbytes, log, error);
            if (result == 0) {
                ngLogError(log, NG_LOGCAT_NINFG_PROTOCOL, fName,  
    ...
    int *error)
{
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiFileReceivingStreamManager_t, rsMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
    ...
    int *error)
{
    ...
    event = rsMng->ngfrsm_protocol->ngp_event;
    ...
    rsMng->ngfrsm_handle = handle;

    /* Open File */
    /* Specified permission for OUT mode file  */
    result = ngiIOhandleFileOpen(
        handle, fileName, NGI_IOHANDLE_FILE_OPEN_TYPE_WRITE, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Open file \"%s\" for write failed.\n", fileName); 
        goto error;
    }
    ...
    int *error)
{
    ...
    if (rsMng->ngfrsm_handleInitialized != 0) {
        rsMng->ngfrsm_handleInitialized = 0;
        /* Destruct the handle */
        result = ngiIOhandleDestruct(rsMng->ngfrsm_handle, log, error);
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Destruct the I/O handle failed.\n");
            return 0;
        }
        rsMng->ngfrsm_handle = NULL;
    }
...
ngiStreamManagerTypeInformation_t nglConversionMethodStreamManagerTypeInformation = {
    /* Destroy Data */
    nglNonImplementationStreamManagerDestroyWriteData,
    nglNonImplementationStreamManagerDestroyReadData,
    ...
    int *error) 
{
    ...
    /* Allocate */
    sMng = NGI_ALLOCATE(ngiConversionMethodStreamManager_t, log, error);
    if (sMng == NULL) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't allocate the storage for Stream Manager.\n"); 
	return NULL;
    ...
    /* Error occurred */
error:
    result = NGI_DEALLOCATE(ngiConversionMethodStreamManager_t, sMng, log, NULL);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
    }
    ...
    int *error)
{
    ...
    result = ngiStreamManagerInitialize((ngiStreamManager_t *)csMng,
        &nglConversionMethodStreamManagerTypeInformation, log, error);
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't initialize the Stream Manager.\n"); 
        return 0;
    ...
    csMng->ngcsm_xdrConversionMethod = ngiMalloc(
	csMng->ngcsm_xdrSize * 3, log, error);
...
error:
    ...
    if (result == 0) {
        ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
            "Can't finalize the ConversionMethod Stream Manager.\n"); 
    }
    return 0;
}
    ...
    int *error)
{
    ...
    if (csMng->ngcsm_xdrConversionMethod != NULL) {
        ngiFree(csMng->ngcsm_xdrConversionMethod, log, error);
    }
    ...
    int *error)
{
    ...
    assert(buf != NULL);
    assert(sMng != NULL);
    assert(sMng->ngsm_typeInformation == &nglConversionMethodStreamManagerTypeInformation);
    ...
    if (csMng->ngcsm_xdrSize > csMng->ngcsm_xdrWriteNbytes) {
        ...
        *nBytes = writableNbytes;
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get writable buffer from the Stream Buffer.\n"); 
            return  0;
        }
    }
    ...
    if (csMng->ngcsm_xdrWriteNbytes < csMng->ngcsm_xdrSize) {
        ...
        if ((csMng->ngcsm_xdrSize == csMng->ngcsm_xdrWriteNbytes) &&
            ...
            default:
                NGI_SET_ERROR(error, NG_ERROR_PROTOCOL);
                ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                    "Can't invalid conversion method.\n"); 
                goto error;
                break;
            }
        }
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get write buffer to the Stream Buffer.\n"); 
            goto error;
        }
    }
    ...
    int *error)
{
    ...
    if ((csMng->ngcsm_xdrWriteNbytes < csMng->ngcsm_xdrSize) ||
        ...
        *nBytes = readableNbytes;
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get readable buffer to the Stream Buffer.\n"); 
            goto error;
        }
    }
    return 1;
    ...
    int *error)
{
    ...
    if ((csMng->ngcsm_xdrWriteNbytes < csMng->ngcsm_xdrSize) ||
        ...
        if (result == 0) {
            ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
                "Can't get read buffer to the Stream Buffer.\n"); 
            goto error;
        }
    }
    ...
    int *error)
{
    ...
    /* Deallocate */
    result = NGI_DEALLOCATE(ngiConversionMethodStreamManager_t, csMng, log, error);
    if (result == 0) {
    	ngLogError(log, NG_LOGCAT_NINFG_PURE, fName,  
    	    "Can't deallocate the storage for Stream Manager.\n"); 
	return 0;
