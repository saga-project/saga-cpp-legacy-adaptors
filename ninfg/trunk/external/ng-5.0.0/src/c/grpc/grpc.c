/*
 * $RCSfile: grpc.c,v $ $Revision: 1.19 $ $Date: 2008/02/07 10:26:15 $
 * $AIST_Release: 5.0.0 $
 * $AIST_Copyright:
...
NGI_RCSID_EMBED("$RCSfile: grpc.c,v $ $Revision: 1.19 $ $Date: 2008/02/07 10:26:15 $")
    ...
    char *config_file_name)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
    ...
    if (grpc_l_context == NULL) {
        ...
        ngclLogErrorContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to construct the Ninf-G Context.\n"); 
        goto error;
    }
    ...
    if (result == 0) {
        ...
        ngclLogErrorContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't initialize the Exclusive lock.\n"); 
        goto error;
    }
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to initialize the Session Queue.\n"); 
        goto error;
    }

    ngclLogDebugContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
        "API %s() return.\n", fName); 
    ...
    void)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to destruct the Session Queue.\n"); 
        return error_code;
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't finalize the Exclusive lock.\n"); 
        return error_code;
    }
    ...
    if (result == 0) {
        ...
        ngclLogErrorContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to destruct the Ninf-G context.\n"); 
        return error_code;
    }
    ...
    char *config_file_name)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to read the Ninf-G Configuration file.\n"); 
        return error_code;
    }
    ...
    grpc_handle_attr_t_np *attr)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get handle attribute.\n"); 
        return error_code;
    }
    ...
    grpc_handle_attr_t_np *attr)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Get Attribute of Ninf-G Executable.\n"); 
        return error_code;
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Release Attribute of Ninf-G Executable.\n"); 
        return error_code;
    }
    ...
    void **attr_value)
{
    ...
    if (attr_value == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Handle attribute value is not valid.\n"); 
        return error_code;
    }
    ...
    *attr_value = NULL;
    ...
    case GRPC_HANDLE_ATTR_HOSTNAME:
        if (attr->gha_remoteHostName != NULL) {
            ...
            if (value == NULL) {
                error_code = GRPC_OTHER_ERROR_CODE;
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "Can't allocate the storage for host name.\n"); 
                return error_code;
            }
        } else {
            value = NULL;
    ...
    case GRPC_HANDLE_ATTR_PORT:
        value = grpc_l_calloc(1, sizeof(int));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for handle attribute.\n"); 
            return error_code;
        }
    ...
    case GRPC_HANDLE_ATTR_JOBMANAGER:
        if (attr->gha_jobManager != NULL) {
            ...
            if (value == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't allocate the storage for jobmanager.\n");
                return error_code;
            }
        } else {
            value = NULL;
    ...
    case GRPC_HANDLE_ATTR_SUBJECT:
        if (attr->gha_subject != NULL) {
            ...
            if (value == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't allocate the storage for Subject.\n");
                return error_code;
            }
        } else {
            value = NULL;
    ...
    case GRPC_HANDLE_ATTR_FUNCNAME:
        if (attr->gha_functionName != NULL) {
            ...
            if (value == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't allocate the storage for func name.\n");
                return error_code;
            }
        } else {
            value = NULL;
    ...
    case GRPC_HANDLE_ATTR_WAIT_ARG_TRANSFER:
        value = grpc_l_calloc(1, sizeof(grpc_argument_transfer_t_np));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
    ...
    case GRPC_HANDLE_ATTR_QUEUENAME:
        if (attr->gha_queueName != NULL) {
            ...
            if (value == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't allocate the storage for queue name.\n"); 
                return error_code;
            }
        } else {
            value = NULL;
    ...
    default:
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Value of Handle attribute name is not valid.\n"); 
        return error_code;
    }
    ...
    void *attr_value)
{
    ...
    if (attr_value == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Value for Handle attribute is not valid.\n"); 
        return error_code;
    }
    ...
    case GRPC_HANDLE_ATTR_HOSTNAME:
        ...
        if (attr->gha_remoteHostName != NULL) {
            grpc_l_free(attr->gha_remoteHostName);
        ...
        if (attr->gha_remoteHostName == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for remote host name.\n"); 
            return error_code;
        }
        break;
    ...
    case GRPC_HANDLE_ATTR_PORT:
        ...
        if (given_value_int < 0) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "The value for handle attribute port"
                " is too small (%d).\n", given_value_int); 
            return error_code;
        }
    ...
    case GRPC_HANDLE_ATTR_JOBMANAGER:
        /* Release the previous value */
        if (attr->gha_jobManager != NULL) {
            grpc_l_free(attr->gha_jobManager);
        ...
        if (attr->gha_jobManager == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for jobmanager.\n"); 
            return error_code;
        }
        break;
    ...
    case GRPC_HANDLE_ATTR_SUBJECT:
        ...
        if (attr->gha_subject == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for Subject.\n"); 
            return error_code;
        }
        break;
    ...
    case GRPC_HANDLE_ATTR_FUNCNAME:
        /* Release the previous value */
        if (attr->gha_functionName != NULL) {
            grpc_l_free(attr->gha_functionName);
        ...
        if (attr->gha_functionName == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for function name.\n"); 
            return error_code;
        }
        break;
    ...
    case GRPC_HANDLE_ATTR_JOBSTARTTIMEOUT:
        ...
        if (given_value_int < 0) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "The value for handle attribute job start timeout"
                " is too small (%d).\n", given_value_int); 
            return error_code;
        }
    ...
    case GRPC_HANDLE_ATTR_JOBSTOPTIMEOUT:
        ...
        if (given_value_int < -1) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "The value for handle attribute job stop timeout"
                " is too small (%d).\n", given_value_int); 
            return error_code;
        }
    ...
    case GRPC_HANDLE_ATTR_WAIT_ARG_TRANSFER:
        ...
        if (!((given_value_transfer == GRPC_ARGUMENT_TRANSFER_WAIT) ||
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Invalid value for handle attribute argument transfer"
                " (%d).\n", (int)given_value_transfer); 
            return error_code;
        }
    ...
    case GRPC_HANDLE_ATTR_QUEUENAME:
        /* Release the previous value */
        if (attr->gha_queueName != NULL) {
            grpc_l_free(attr->gha_queueName);
        ...
        if (attr->gha_queueName == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for queue name.\n"); 
            return error_code;
        }
        break;
    ...
    case GRPC_HANDLE_ATTR_MPI_NCPUS:
        ...
        if (given_value_int < 1) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "The value for handle attribute MPI number of cpus"
                " is too small (%d).\n", given_value_int); 
            return error_code;
        }
    ...
    grpc_session_attr_t_np *attr)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to initialize the Session Attribute.\n"); 
        return error_code;
    }
    ...
    grpc_session_attr_t_np *attr)
{
    ...
    if (attr == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Session attribute is not valid.\n"); 
        return error_code;
    }
    ...
    void **attr_value)
{
    ...
    if (attr_value == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Session attribute value is not valid.\n"); 
        return error_code;
    }
    ...
    case GRPC_SESSION_ATTR_WAIT_ARG_TRANSFER:
        ...
        if (value == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't allocate the storage for session attribute.\n"); 
            return error_code;
        }
        /* Copy flag of wait for the end of argument transfer or not */
    ...
    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_ARGUMENT:
        value = grpc_l_calloc(1, sizeof(int));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
        ...
        /* Copy flag of transfer timeout */
        *((int *)value) = attr->gsa_transferTimeout_argument;
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_RESULT:
        value = grpc_l_calloc(1, sizeof(int));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
        ...
        /* Copy flag of transfer timeout */
        *((int *)value) = attr->gsa_transferTimeout_result;
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_CALLBACK_ARGUMENT:
        value = grpc_l_calloc(1, sizeof(int));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
        ...
        /* Copy flag of transfer timeout */
        *((int *)value) = attr->gsa_transferTimeout_cbArgument;
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_CALLBACK_RESULT:
        value = grpc_l_calloc(1, sizeof(int));
        if (value == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
            grpc_l_error_set(error_code);
        ...
        /* Copy flag of transfer timeout */
        *((int *)value) = attr->gsa_transferTimeout_cbResult;
        break;
    ...
    default:
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Value of Session attribute name is not valid.\n"); 
        return error_code;
    }
    ...
    void *attr_value)
{
    ...
    if (attr_value == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Value for Session attribute is not valid.\n"); 
        return error_code;
    }
    ...
    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_ARGUMENT:
        /* Copy flag of transfer timeout */
        attr->gsa_transferTimeout_argument = *((int *)attr_value);
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_RESULT:
        /* Copy flag of transfer timeout */
        attr->gsa_transferTimeout_result = *((int *)attr_value);
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_CALLBACK_ARGUMENT:
        /* Copy flag of transfer timeout */
        attr->gsa_transferTimeout_cbArgument = *((int *)attr_value);
        break;

    case GRPC_SESSION_ATTR_TRANSFER_TIMEOUT_CALLBACK_RESULT:
        /* Copy flag of transfer timeout */
        attr->gsa_transferTimeout_cbResult = *((int *)attr_value);
        break;
    ...
    int maxsize)
{
    ...
    if (stack == NULL) {
        ...
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to construct the Argument Stack.\n"); 
        return NULL;
    }
    ...
    grpc_arg_stack_t *stack)
{
    ...
    if (stack == NULL) {
        ...
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Argument stack is not valid.\n"); 
        return -1;
    }

    /* Argument stack destruct by Pure API */
    result = ngclArgumentStackDestruct(grpc_l_context, stack, &error);
    if (result == 0) {
        result = grpc_i_get_error_from_ng_error(error);
        grpc_l_error_set(result);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to destruct the argument Stack.\n"); 
        return -1;
    }
    ...
    void *arg)
{
    ...
    result = ngclArgumentStackPush(grpc_l_context, stack, arg, &error);
    if (result == 0) {
        result = grpc_i_get_error_from_ng_error(error);
        grpc_l_error_set(result);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to push onto Argument Stack.\n"); 
        return -1;
    }
    ...
    grpc_arg_stack_t *stack)
{
    ...
    arg = ngclArgumentStackPop(
	grpc_l_context, ((ngclArgumentStack_t *)stack), &error);
#if 0 /* 2004/9/8 asou
       * Is this necessary? Does anybody specify a NULL argument?
       */
    if (arg == NULL) {
        ...
        grpc_l_error_set(result);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to pop from Argument Stack.\n"); 
        return NULL;
    }
#endif
    ...
    grpc_sessionid_t sessionId)
{
    ...
    result = ngclNinfgManagerYieldForCallback(grpc_l_context, NULL);
    if (result == 0) {
        error_code = GRPC_OTHER_ERROR_CODE;
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't yield for I/O callback.\n"); 
        return error_code;
    }
    ...
    if (ret != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't read lock the Session Queue.\n"); 
        return ret;
    }
    ...
    if (ret != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't read unlock the Session Queue.\n"); 
        return ret;
    }
    ...
    if (ret != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to release Ninf-G Session.\n"); 
        return ret;
    }
    ...
    int *status)
{
    ...
    if (info != NULL) {
	*info = grpc_l_calloc(1, sizeof(grpc_session_info_t_np));
	if (*info == NULL) {
	    error_code = GRPC_OTHER_ERROR_CODE;
	    grpc_l_error_set(error_code);
	    ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
...
grpc_session_info_release_np(grpc_session_info_t_np *info)
{
	...
	if (info->gei_compressionInformation.toClient != NULL)
	    grpc_l_free(info->gei_compressionInformation.toClient);
	info->gei_compressionInformation.toClient = NULL;

	if (info->gei_compressionInformation.toRemote != NULL)
	    grpc_l_free(info->gei_compressionInformation.toRemote);
    ...
    grpc_sessionid_t sessionId)
{
    ...
    if (result == -1) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get session error code.\n"); 
        return error_code;
    }
    ...
    grpc_sessionid_t sessionId)
{
   ...
   if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to unregister Ninf-G Session.\n"); 
        return error_code;
    }
    ...
    grpc_sessionid_t *id)
{
    ...
    if (id == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Session ID pointer is not valid.\n"); 
        return error_code;
    }
    ...
    int threshold)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Set the session_info threshold failed.\n"); 
        return error_code;
    }
    ...
    void)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
        ...
        if (result == 0) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't lock the last error lock.\n"); 
            return error_code;
        }
        ...
        if (result == 0) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't unlock the last error lock.\n"); 
            return error_code;
        }
    }
    ...
    void)
{
    ...
    if (result != 0) {
        ...
        grpc_l_last_error.ge_message =
            grpc_l_error_message[grpc_l_last_error.ge_code];
        ngclLogFatalContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
    ...
    if (grpc_l_initialized != 0) {
        ...
        ngclLogErrorContext(NULL, NG_LOGCAT_NINFG_GRPC, fName,  
            "GRPC client has already been initialized.\n"); 
    ...
    void)
{
    ...
    if (result != 0) {
        ...
        grpc_l_last_error.ge_message =
            grpc_l_error_message[grpc_l_last_error.ge_code];
        ngclLogFatalContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to pthread_mutex_lock.\n"); 
        return error_code;
    }
#endif /* NG_PTHREAD */
    ...
    grpc_handle_attr_t_np *attr)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Initialize the Attribute of Ninf-G Executable.\n"); 
        goto error;
    }
    ...
    if (exeAttr.ngea_className == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't duplicate the string for class name.\n"); 
        goto error;
    }
    ...
    if (attr != NULL) {
        if (attr->gha_remoteHostName != NULL) {
            ...
            if (exeAttr.ngea_hostName == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't duplicate the string for host name.\n"); 
                goto error;
            }
        }
        ...
        if (attr->gha_jobManager != NULL) {
            ...
            if (exeAttr.ngea_jobManager == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "Can't duplicate the string for jobmanager.\n"); 
                goto error;
            }
        }
        ...
        if (attr->gha_subject != NULL) {
            ...
            if (exeAttr.ngea_subject == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "Can't duplicate the string for Subject.\n"); 
                goto error;
            }
        }
        ...
        if (attr->gha_queueName != NULL) {
            ...
            if (exeAttr.ngea_queueName == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                    "Can't duplicate the string for queue name.\n"); 
                goto error;
            }
        }
        ...
        if (server_name != NULL) {
            ...
            /* Copy the hostname */
            exeAttr.ngea_hostName = strdup(server_name);
            if (exeAttr.ngea_hostName == NULL) {
                error_code = GRPC_OTHER_ERROR_CODE;
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                    "Can't duplicate the string for server name \"%s\".\n",
                    server_name); 
                goto error;
            }
        } 
    ...
    if (executable == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to construct the Ninf-G Executable.\n"); 
        goto error;
    }
    ...
    result = ngclRemoteMachineInformationGetCopy(
        grpc_l_context, executable->nge_hostName, &rmInfo, &error);
    if (result == 0) {
        error_code = grpc_i_get_error_from_ng_error(error);
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Get Remote Machine Information.\n"); 
        goto error;
    }
    ...
    if (saveAttr.gha_remoteHostName == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't duplicate the string for server name.\n"); 
        goto error;
    }
    ...
    if (rmInfo.ngrmi_jobManager != NULL) {
        ...
        if (saveAttr.gha_jobManager == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't duplicate the string for jobmanager.\n"); 
            goto error;
        }
    }
    ...
    if (rmInfo.ngrmi_subject != NULL) {
        ...
        if (saveAttr.gha_subject == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't duplicate the string for Subject.\n"); 
            goto error;
        }
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Release Remote Machine Information.\n"); 
        goto error;
    }
    ...
    if (exeAttr.ngea_jobManager != NULL) {
        if (saveAttr.gha_jobManager != NULL) {
            grpc_l_free(saveAttr.gha_jobManager);
    ...
    if (exeAttr.ngea_subject != NULL) {
        if (saveAttr.gha_subject != NULL) {
            grpc_l_free(saveAttr.gha_subject);
    ...
    if (exeAttr.ngea_queueName != NULL) {
        if (saveAttr.gha_queueName != NULL) {
            grpc_l_free(saveAttr.gha_queueName);
    ...
    grpc_handle_attr_t_np *attr)
{
    ...
    *(exec_handle->geh_errorMessage) = strdup("No error");
    if (*(exec_handle->geh_errorMessage) == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't allocate the storage for handle error message.\n"); 
        return error_code;
    }

    newAttr = (grpc_handle_attr_t_np *)
        grpc_l_calloc(1, sizeof(grpc_handle_attr_t_np));
    if (newAttr == NULL) {
        error_code = GRPC_OTHER_ERROR_CODE;
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
    ...
    if (error_code != GRPC_NO_ERROR) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't copy the handle attribute.\n"); 
        return error_code;
    }
    ...
    *(exec_handle->geh_attr) = newAttr;
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to register user data to Ninf-G Executable.\n"); 
        ...
        if (ret != GRPC_NO_ERROR) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Failed to unregister user data from Ninf-G Executable.\n"); 
            goto error;
        }
        goto error;
    }
    ...
    grpc_l_executable_handle_t *exec_handle)
{
    ...
    if (error_code != GRPC_NO_ERROR) {
	ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
	    "Failed to unregister the Session.\n"); 
	goto error;
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to destruct the Ninf-G Executable.\n"); 
        goto error;
    }
    ...
    grpc_l_executable_handle_t *exec_handle, int *unusedSessions)
{
    ...
    *unusedSessions = 0;
	...
	if (result == 0) {
	    ...
	    grpc_l_error_set(error_code);
	    ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
	        "Failed to get user data from Ninf-G Session.\n"); 
	    goto error;
	}
	...
	*unusedSessions = 1;
	...
	if (result == 0) {
	    ...
	    grpc_l_error_set(error_code);
	    ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
	        "Failed to Unregist Session from Ninf-G Executable.\n"); 
	    goto error;
	}
	...
	if (ret != GRPC_NO_ERROR) {
	    ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
	        "Failed to register Session to Session Queue.\n"); 
	    return ret;
	}

	ngclLogWarnSession(session, NG_LOGCAT_NINFG_GRPC, fName,  
	    "Session was unregistered from Executable.\n"); 
    ...
    grpc_handle_attr_t_np *attr)
{
    ...
    if (handleAttr == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Handle attribute in the handle is not valid.\n"); 
        return error_code;
    }
    ...
    grpc_l_handle_type_t handle_type)
{
    ...
    if (result != 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogInfoContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "This handle is not ready.\n"); 
        return error_code;
    }
    ...
    grpc_object_handle_t_np **object_handle)
{
    ...
    if (executable == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get Ninf-G Executable.\n"); 
        goto error;
    }
    ...
    if ((result == 0) || (handle == NULL)) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get user data from Ninf-G Executable.\n"); 
        goto error;
    }
    ...
    grpc_l_executable_handle_t *exec_handle)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(result);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to lock Session management queue.\n"); 
        return error_code;
    }
    ...
    *(exec_handle->geh_errorMessage) = NULL;
    ...
    if (error_message == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't duplicate the string for error message.\n"); 
    }
    *(exec_handle->geh_errorMessage) = error_message;
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to unlock Session management queue.\n"); 
        return error_code;
    }
    ...
    void)
{
    ...
    if ((handleIdConter + 1) >= GRPC_L_HANDLE_ID_MAX) {
        grpc_l_error_set(GRPC_OTHER_ERROR_CODE);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "There is no IDs for Handle to use.\n"); 
    ...
    *dstAttr = *srcAttr;
    ...
    if (srcAttr->gha_remoteHostName != NULL) {
        ...
        if (dstAttr->gha_remoteHostName == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't duplicate the string for host name.\n"); 
            return error_code;
        }
    }
    ...
    if (srcAttr->gha_functionName != NULL) {
        ...
        if (dstAttr->gha_functionName == NULL) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Can't duplicate the string for func name.\n"); 
            return error_code;
        }
    }
    ...
    grpc_object_handle_t_np *object_handle)
{
    ...
    if (handle_type == GRPC_L_HANDLE_TYPE_FUNCTION) {
        ...
        if (function_handle->gfh_discernment != GRPC_HANDLE_TYPE_FUNCTION) {
            ...
            grpc_l_error_set(error_code);
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "This handle is not the Function handle.\n"); 
            return error_code;
        }
    ...
    grpc_session_attr_t_np *session_attr)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Get Attribute of Session.\n"); 
        goto error;
    }
    ...
    if (session_attr != NULL) {
        ...
        ngclSessionAttr.ngsa_transferTimeout_argument =
            session_attr->gsa_transferTimeout_argument;
        ngclSessionAttr.ngsa_transferTimeout_result =
            session_attr->gsa_transferTimeout_result;
        ngclSessionAttr.ngsa_transferTimeout_cbArgument =
            session_attr->gsa_transferTimeout_cbArgument;
        ngclSessionAttr.ngsa_transferTimeout_cbResult =
            session_attr->gsa_transferTimeout_cbResult;
    }
    ...
    if (session == NULL) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to construct the Ninf-G Session.\n"); 
        goto error;
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to Release Attribute of Session.\n"); 
        goto error;
    }

    /* Allocate the storage for Session management data */
    session_data = grpc_l_calloc(1, sizeof (*session_data));
    if (session_data == NULL) {
        error_code = GRPC_OTHER_ERROR_CODE;
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't allocate the storage for Session management data.\n"); 
        goto error;
    }
    ...
    if (result == 0) {
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to register user data to Ninf-G Session.\n"); 
	goto error;
    }
    ...
    grpc_sessionid_t *id_ptr)
{
	...
	assert(given_session == NULL);
        ...
        if (error_code != GRPC_NO_ERROR) {
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "Failed to get Ninf-G Session.\n"); 
                goto error;
        }
        ...
        /* Given Session ID*/
            ...
            if (id_length <= 0) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "argument is invalid."
                    " session array should be larger than 0."
                    " (length == %lu)\n",
             ...
             if (id_array == NULL) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "argument is invalid. session array is NULL\n"); 
                goto error;
            }
    ...
    if (error_code != GRPC_NO_ERROR) {
        ...
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Session wait returned by error.\n"); 
        ...
        for (current_session = session_list; current_session != NULL;
            ...
            if ((current_session->ngs_status == NG_SESSION_STATUS_DONE) &&
                ...
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName, "The error occurred for session %d. (reason=%d)\n",
                    current_session->ngs_ID, current_session->ngs_error); 
        ...
        if ((wait_type == GRPC_L_WAIT_TYPE_ANY) && (ret_id < 0)) {
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Failed to find the error session.\n"); 
            goto error;
        }
    }
    ...
    if (wait_type == GRPC_L_WAIT_TYPE_ANY) {
        ...
        *id_ptr = ret_id;
        ...
        if (session_list == NULL) {
            ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
                "Failed to get Ninf-G Session %d.\n", ret_id); 
            return ret;
        }
    }
...
grpc_l_wait_all(ngclExecutable_t *executable, ngclSession_t *session_list)
{
	...
	for (curr = session_list; curr != NULL; curr = next) {
		...
		if (curr->ngs_error != NG_ERROR_NO_ERROR) {
		    ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
		        "The error occurred for session %d. (reason=%d)\n",
                        curr->ngs_ID, curr->ngs_error); 
		}
	    }
	}
    ...
    grpc_error_t *error_code)
{
    ...
    if (executable == NULL) {
        *error_code = grpc_i_get_error_from_ng_error(error);
        grpc_l_error_set(*error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get Ninf-G Executable from Ninf-G Session.\n"); 
        return NULL;
    }
    ...
    grpc_session_attr_t_np *attr)
{
    ...
    attr->gsa_transferTimeout_argument =
        sessionAttr.ngsa_transferTimeout_argument;
    attr->gsa_transferTimeout_result =
        sessionAttr.ngsa_transferTimeout_result;
    attr->gsa_transferTimeout_cbArgument =
        sessionAttr.ngsa_transferTimeout_cbArgument;
    attr->gsa_transferTimeout_cbResult =
...
/**
 * Get Session Information from Session
 */
grpc_error_t
    ...
    grpc_session_info_t_np *info)
{
    ...
    if (session->ngs_info.ngsi_nCompressionInformations > 0) {
        info->gei_compressionInformation.toRemote = grpc_l_calloc(
    ...
    if (session->ngs_info.ngsi_nCompressionInformations > 0) {
        info->gei_compressionInformation.toClient = grpc_l_calloc(
            session->ngs_info.ngsi_nCompressionInformations,
            sizeof (grpc_compression_info_t_np));
        if (info->gei_compressionInformation.toClient == NULL) {
            error_code = GRPC_OTHER_ERROR_CODE;
...
/**
 * Copy Session Information from Session.
 */
static void
...
/**
 * Copy Session Information Element.
 */
static void
grpc_l_session_info_copy_element(
    ...
    grpc_l_session_queue_t *session_queue)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to get Local Machine Information.\n"); 
        return error_code;
    }
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Failed to release Local Machine Information.\n"); 
        return error_code;
    }
    ...
    grpc_l_session_queue_t *session_queue)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't unlock the Session Queue.\n"); 
        return error_code;
    }
    ...
    grpc_l_session_queue_t *session_queue)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't write lock the Session Queue.\n"); 
        return error_code;
    }
    ...
    grpc_l_session_queue_t *session_queue)
{
    ...
    if (result == 0) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't write unlock the Session Queue.\n"); 
        return error_code;
    }
    ...
    grpc_sessionid_t sessionID)
{
    ...
    while (loop_continue != 0) {
        ...
        if (remove_this != 0) {
            ...
            if (result == 0) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "Failed to finalize Ninf-G session.\n"); 
                /* Not Return */
            }
            ...
            if (result == 0) {
                ...
                grpc_l_error_set(error_code);
                ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC,
                    fName,  "Failed to free Ninf-G session.\n"); 
                /* Not Return */
            }
    ...
    if ((remove_by_sessionid != 0) && (removed == 0)) {
        ...
        grpc_l_error_set(error_code);
        ngclLogErrorContext(grpc_l_context, NG_LOGCAT_NINFG_GRPC, fName,  
            "Can't find the session by ID %d.\n", sessionID); 
        goto error;
    }
    ...
    size_t elementSize)
{
    ...
    ret = ngiCalloc(nElements, elementSize, grpc_l_context->ngc_log, NULL);
...
grpc_l_free(
    void *target)
{
    ngiFree(target, grpc_l_context->ngc_log, NULL);
}
