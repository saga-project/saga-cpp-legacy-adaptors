 ...
 * rns-chgrp
 */
public class ACL_chgrp {
	public static void main(String[] args) {
		RNSClientHome home = new RNSClientHome();
		home.setCustomUsage("group RNS_path");
		boolean usage = true;
		try {
			List<String> al = home.parseArgs(args, 2, 2);
			String group = al.get(0);
			String path = al.get(1);
			usage = false;
			RNSExtensionClient client = home.getRNSExtensionClient();
			ACL acl = client.getACL(path, false);
			client.setACL(path, "og:" + group + ":" + ACL.permToString(acl.getOwnerGroupPerm()));
		} catch (Exception e) {
