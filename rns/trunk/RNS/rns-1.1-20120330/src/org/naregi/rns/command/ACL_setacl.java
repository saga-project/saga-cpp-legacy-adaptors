...
public class ACL_setacl {
	public static void main(String[] args) {
		...
		home.setCustomUsage("ACL_specs RNS_path");
		boolean usage = true;
		try {
			List<String> al = home.parseArgs(args, 2, 2);
			String aclSpec = al.get(0);
			String path = al.get(1);
			usage = false;
			RNSExtensionClient client = home.getRNSExtensionClient();
			client.setACL(path, aclSpec);
		} catch (Exception e) {
			home.printError(e, System.err);
			if (usage) {
				home.printUsage(System.out);
				System.out.println("ACL_specs: [default(d):]owner(ou)|ownergroup(og)|user(u)|group(g)|mask(m)|other(o):[NAME]:perms(r|w|x)[,...]");
			}
			System.exit(1);
			return;
		}
