...
public class AccessControlVOMS implements AccessControl {
	...
	private static boolean debug = RNSConfig.debug();
	...
	private static class VOMSCallerInfo implements CallerInfo {
		...
		public VOMSCallerInfo() {
			...
			try {
				...
				if (s != null) {
					if (debug) {
						System.out.println("*** [DEBUG] VOMS informations ***");
						System.out.println("subject: " + s);
					}
					Set<?> publicCreds = s
							.getPublicCredentials(X509Certificate[].class);
					Iterator<?> iter = publicCreds.iterator();
					String vo = null;
					String hostport = null;
					while (iter.hasNext()) {
						X509Certificate certRev[] = (X509Certificate[]) iter
								.next();
						...
						if (vector.size() != 0) {
							for (int j = 0; j < vector.size(); j++) {
								VOMSAttribute attrib = (VOMSAttribute) vector
										.get(j);
								if (debug) {
									vo = attrib.getVO();
