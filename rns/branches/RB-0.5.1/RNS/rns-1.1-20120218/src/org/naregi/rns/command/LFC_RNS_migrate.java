...
public class LFC_RNS_migrate {
	...
	private static void warning(String msg) {
		System.err.println("LFC_RNS_import: warning: " + msg);
	}
	...
	private static MessageElement[] myMetadata(RNSClientHome home,
		...
		String xml = "<file><size>" + fileSize + "</size>" + "<mtime>" + mtime
				+ "</mtime><replica>";
		int no = 0;
	...
	public static void createRecursive(LFCServer lfcs, RNSClientHome home,
		...
		for (FileDesc fd : dir) {
			...
			if (fd.isDirectory()) {
				...
				if (fd.getULink() > 0) {
					createRecursive(lfcs, home, lfcPath + "/" + name, newPath,
							verbose);
				}
			} else if (fd.isFile()) {
	...
	public static void createRecursiveBulk(LFCServer lfcs, RNSClientHome home,
			String lfcPath, String rnsPath, boolean verbose)
			throws LFCException, RNSError {
		RNSClient rnsc = home.getRNSClient();
		ArrayList<FileDesc> dir = lfcs.listDirectory(lfcPath);
		...
		for (String childDirName : mkdirList) {
			createRecursiveBulk(lfcs, home, lfcPath + "/" + childDirName,
					rnsPath + "/" + childDirName, verbose);
		}
	}
		...
		try {
			...
			if (entfd.isDirectory()) {
				...
				if (argMap.containsKey("+b") || argMap.containsKey("++bulk")) {
					createRecursiveBulk(lfcs, home, lfnUri.getPath(), rnsPath,
							verbose);
				} else {
					createRecursive(lfcs, home, lfnUri.getPath(), rnsPath,
							verbose);
				}
			} else {
				/* copy a junction */
