...
#ifndef ADAPTORS_LSF_JOB_HPP
#define ADAPTORS_LSF_JOB_HPP
...
namespace saga { namespace adaptors { namespace lsf
{
    ...
    : public saga::adaptors::v1_0::job_cpi <job_cpi_impl>
  {
    private:
      ...
      void set_job_attributes(std::string job_id);
      void update_state(saga::job::state newstate);
      saga::job::state get_rm_job_state(std::string job_id);
      std::vector<std::string> jd_to_arg_list(const saga::job::description &jd);
