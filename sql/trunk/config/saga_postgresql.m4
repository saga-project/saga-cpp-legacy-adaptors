    ...
    for tmp_path in $tmp_location $POSTGRESQL_LOCATION /usr /usr/local /usr/local/pgsql /opt /opt/local $packages; do
      ...
      if test "x$have_something" = "x"; then
        ...
        if test "x$have_something" = "x"; then
          ...
          libdir=`echo $have_something | head -1 | rev | cut -f 3 -d '/' | rev`
        fi
      else
        libdir=`echo $have_something | head -1 | rev | cut -f 2 -d '/' | rev`
      fi
      ...
      POSTGRESQL_LDFLAGS="-L$tmp_path/$libdir/$subdir -lpq"
      ...
      LD_LIBRARY_PATH="$tmp_path/$libdir/$subdir:$LD_LIBRARY_PATH"
      DYLD_LIBRARY_PATH="$tmp_path/$libdir/$subdir:$DYLD_LIBRARY_PATH"
      ...
      if test "x$link_postgresql" = "xno"; then
        ...
        if test -e "$tmp_path/$libdir/$subdir/libpq.a"; then
          POSTGRESQL_S_LIBS="$tmp_path/$libdir/$subdir/libpq.a"
