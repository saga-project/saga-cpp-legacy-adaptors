...
[
  ...
  if test "$FAILED" -eq "0"; then
    ...
    GLOBUS_INCDIRS=`grep GLOBUS_INCLUDES globus_defines | sed -e 's/^GLOBUS_INCLUDES *= *-I//' | sed -e 's/-I//g'`
    ...
    # release...
    GLOBUS_INCDLUDES=""
    for d in $GLOBUS_INCDIRS; do
      GLOBUS_INCLUDES="$GLOBUS_INCLUDES -I$d/ -I$d/$GLOBUS_FLAVOR/"
    done
    GLOBUS_FIRSTINCDIR=`echo $GLOBUS_INCDIRS | sed -e 's/ .*$//g'`
  ...
  if test "$FAILED" -eq "0"; then
    ...
    SAGA_GLOBUS_DEP_FILES="$GLOBUS_FIRSTINCDIR/globus_common.h"
    ...
    if test -z "$GLOBUS_HAVE_GRAM" ; then
      ...
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_gram_client.h"
    fi
    ...
    if test -z "$GLOBUS_HAVE_GRIDFTP" ; then
      ...
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_ftp_client.h"
    fi
    ...
    if test -z "$GLOBUS_RLS_CLIENT_VERSION" ; then
      ...
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_rls_client.h"
    fi
    ...
    if test -z "$GLOBUS_GSI_CLIENT_VERSION" ; then
      ...
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_gsi_proxy.h"
    fi
    ...
    if test -z "$GLOBUS_HAVE_GASS_COPY" ; then
      ...
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_gass_copy.h"
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_gass_server.h"
      SAGA_GLOBUS_DEP_FILES="$SAGA_GLOBUS_DEP_FILES $GLOBUS_FIRSTINCDIR/globus_gass_transfer.h"
    fi
